...
public class VectorDrawbleGeneratorTest extends TestCase {
    private static final String TEST_DATA_REL_PATH =
            "tools/base/sdk-common/src/test/resources/testData/assets/vectordrawable";
    ...
    private void checkVectorConversion(String testFileName) throws IOException {
        ...
        File root = TestUtils.getRoot("assets", "vectordrawable");
        File currentSvgFile = new File(root, svgName);

        OutputStream outStream = new ByteArrayOutputStream();
        try {
            Svg2Vector.parseSvgToXml(new File(currentSvgFile.toURI()), outStream);
        }
        catch (Exception e) {
            assertTrue("Failure: Exception in Svg2Vector.parseSvgToXml!", false);
        }

        BufferedImage image = VdPreview.getPreviewFromVectorXml(24, 0.0f, outStream.toString(),
                                                                null);
        File goldenImageFile = new File(root, imageName);
        String path = goldenImageFile.getAbsolutePath();
        InputStream is = new FileInputStream(goldenImageFile);
                //VectorDrawbleGeneratorTest.class.getResourceAsStream(path);
        if (is == null) {
            // Generate golden images here.
            generateGoldenImage(root, image, path, imageName);
        } else {
    ...
    // This is a copy paste code from GeneratorTest.java. I hope I can find a better place
    // for simply share this code.
