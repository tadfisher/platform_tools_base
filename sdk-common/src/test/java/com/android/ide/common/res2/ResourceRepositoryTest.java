...
public class ResourceRepositoryTest extends BaseTestCase {
    ...
    public void testBaseStyleValue() throws Exception {
        ...
        ItemResourceValue styleValue = styleResourceValue.findVal("singleLine", true /*framework*/);
        assertNotNull(styleValue);
        assertEquals("true", styleValue.getValue());

        styleValue = styleResourceValue.findVal("textAppearance", true /*framework*/);
        assertNotNull(styleValue);
        assertEquals("@style/TextAppearance.WindowTitle", styleValue.getValue());

        styleValue = styleResourceValue.findVal("shadowColor", true /*framework*/);
        assertNotNull(styleValue);
        assertEquals("#BB000000", styleValue.getValue());

        styleValue = styleResourceValue.findVal("shadowRadius", true /*framework*/);
        assertNotNull(styleValue);
        assertEquals("2.75", styleValue.getValue());

        styleValue = styleResourceValue.findVal("foo", false /*framework*/);
        assertNotNull(styleValue);
                        ...
                        if (value == null || value.isEmpty()) {
                            if (resourceValue instanceof StyleResourceValue) {
                                ...
                                for (String name : srv.getNames()) {
                                    ItemResourceValue value1 = srv.findVal(name, false);
                                    ItemResourceValue value2 = srv.findVal(name, true);
                                    if (value1 != null) {
