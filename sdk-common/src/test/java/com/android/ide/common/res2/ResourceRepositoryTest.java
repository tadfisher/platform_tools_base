...
public class ResourceRepositoryTest extends BaseTestCase {
    ...
    public void testBaseStyleValue() throws Exception {
        ...
        ItemResourceValue styleValue = styleResourceValue.findItem("singleLine", true /*framework*/);
        assertNotNull(styleValue);
        assertEquals("true", styleValue.getValue());

        styleValue = styleResourceValue.findItem("textAppearance", true /*framework*/);
        assertNotNull(styleValue);
        assertEquals("@style/TextAppearance.WindowTitle", styleValue.getValue());

        styleValue = styleResourceValue.findItem("shadowColor", true /*framework*/);
        assertNotNull(styleValue);
        assertEquals("#BB000000", styleValue.getValue());

        styleValue = styleResourceValue.findItem("shadowRadius", true /*framework*/);
        assertNotNull(styleValue);
        assertEquals("2.75", styleValue.getValue());

        styleValue = styleResourceValue.findItem("foo", false /*framework*/);
        assertNotNull(styleValue);
                        ...
                        if (value == null || value.isEmpty()) {
                            if (resourceValue instanceof StyleResourceValue) {
                                ...
                                for (String name : srv.getNames()) {
                                    ItemResourceValue value1 = srv.findItem(name, false);
                                    ItemResourceValue value2 = srv.findItem(name, true);
                                    if (value1 != null) {
