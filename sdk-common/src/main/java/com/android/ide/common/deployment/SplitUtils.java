...
public class SplitUtils {
     ...
     * @param connectedDevice the deployment device.
     * @param allAPKs the split APKs
     * @return a list of APKs that should be deployed on the device.
     */
    public List<File> filter(DeviceConnector connectedDevice, File[] allAPKs) {
        List<ApkInfo> apkInfos = introspect(allAPKs);
        String deviceDensity = getDeviceDensity(connectedDevice);
    ...
    private String getDeviceDensity(DeviceConnector propGetter) {
        return propGetter.getProperty(PROP_DEVICE_DENSITY);
    }
