...
class ValueResourceParser2 {
    ...
    static ResourceType getType(@NonNull Node node, @Nullable File from) throws MergingException {
        ...
        if (typeString != null) {
            ...
            throw new MergingException(String.format("Unsupported type '%s'", typeString)).addFile(from);
        }

        throw new MergingException(String.format("Unsupported node '%s'", nodeName)).addFile(from);
    }
