...
public class LayoutLibrary {
     ...
     *
     * @see Bridge#getFeatures()
     */
    public boolean supports(int capability) {
        if (mBridge != null) {
            if (mBridge.getApiLevel() > 12) {
                // Features were introduced in API level 13.
                return mBridge.getFeatures().contains(capability);
            } else {
                // API Level 12 had 14 capabilities.
                return capability < 14
        ...
        if (mLegacyBridge != null) {
            if (capability == Capability.UNBOUND_RENDERING.ordinal()) {
