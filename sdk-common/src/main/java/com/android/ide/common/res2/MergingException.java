...
public class MergingException extends Exception {
    ...
    public MergingException setFile(@Nullable File file) {
        if (file != null) {
    ...
    public MergingException addFile(@Nullable File file) {
        if(file != null) {
            mFilePositions.add(new FilePosition(file, SourcePositionRange.UNKNOWN));
        }
        return this;
    }
    ...
    public MergingException addFilePosition(@NonNull File file, @NonNull SAXParseException exception) {
        ...
        if (lineNumber != -1) {
            addFilePosition(new FilePosition(file, new SourcePositionRange(
    ...
    public String getMessage() {
        ...
        Iterable<String> filePositionStrings = Iterables.transform(mFilePositions,
                new Function<FilePosition, String>() {
                    @Override
                    public String apply(@Nullable FilePosition input) {
                        return input.toString();
                    }
                });
        sb.append(Joiner.on('\t').join(filePositionStrings));
