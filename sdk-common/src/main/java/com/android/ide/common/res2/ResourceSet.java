...
public class ResourceSet extends DataSet<ResourceItem, ResourceFile> {
    ...
    protected boolean handleChangedFile(@NonNull File sourceFolder, @NonNull File changedFile)
        ...
        if (resourceFile == null) {
            // Maybe the file is in the generated set?
            if (mGeneratedSet.getDataFile(changedFile) != null) {
                return mGeneratedSet.handleChangedFile(sourceFolder, changedFile);
            } else {
        ...
        switch (resourceFile.getType()) {
            ...
            case GENERATED_FILES:
                // TODO: what if it no longer needs preprocessing?
                handleChangedItems(resourceFile, getResourceItemsForGeneratedFiles(changedFile));
                break;
            case XML_VALUES:
