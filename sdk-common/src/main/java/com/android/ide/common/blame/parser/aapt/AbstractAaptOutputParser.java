...
public abstract class AbstractAaptOutputParser implements PatternAwareOutputParser {
    ...
    @NonNull
    private static Position findMessagePositionInFile(@NonNull File file, @NonNull String msgText, int locationLine, ILogger logger) {
        Position exactPosition = findExactMessagePositionInFile(file, msgText, locationLine, logger);
        if (exactPosition != null) {
            return exactPosition;
        } else {
            return new Position(locationLine, -1, -1);
        }
    }

    @Nullable
    private static Position findExactMessagePositionInFile(@NonNull File file, @NonNull String msgText, int locationLine, @NonNull ILogger logger) {
        ...
        if (matcher.find()) {
            ...
            Position position1 = findText(file, name, "\"\"", locationLine, logger);
            Position position2 = findText(file, name, "''", locationLine, logger);
            if (position1 == null) {
                if (position2 == null) {
    ...
    @Nullable
    private static Position findText(@NonNull File file, @NonNull String first,
        ...
        return new Position(startLineNumber, resultOffset - startLineOffset + 1, resultOffset,
                endLineNumber, endResultOffset - endLineOffset + 1, endResultOffset);
    }

    @Nullable
    private static Position findLineStart(@NonNull File file, int locationLine, ILogger logger) {
        ReadOnlyDocument document = getDocument(file, logger);
        if (document == null) {
            return null;
        }
    ...
    GradleMessage createMessage(@NonNull GradleMessage.Kind kind,
        ...
        Position errorPosition = parseLineNumber(lineNumberAsText);
        if (sourcePath != null) {
    ...
    private Position parseLineNumber(String lineNumberAsText) throws ParsingFailedException {
        int lineNumber = -1;
    ...
    @Nullable
    protected FilePosition findSourcePosition(@NonNull File file, int locationLine,
        ...
        if (isValueFile) {
            ...
            Position position = findMessagePositionInFile(sourceFile, message,
        ...
        return new FilePosition(sourceFile, new Position());
    }
    ...
    private static class FilePosition {
        ...
        FilePosition(File sourceFile, Position position) {
