...
public class GradleCoordinate implements Comparable<GradleCoordinate> {
  ...
  private final ArrayList<Integer> myRevisions = new ArrayList<Integer>(3);
   ...
   * @param revisions
   */
  public GradleCoordinate(String groupId, String artifactId, List<Integer> revisions) {
    myGroupId = groupId;
    myArtifactId = artifactId;
    myRevisions.addAll(revisions);

    // If the major revision is "+" then we'll accept any revision
    myIsAnyRevision = (myRevisions.size() > 0 && myRevisions.get(0) == PLUS_REV);
  }
  ...
  public String toString() {
    ...
    revision.deleteCharAt(revision.length() - 1);
  ...
  public String getFullRevision() {
    ...
    for (Integer i : myRevisions) {
      ...
      // Plus symbol can only appear as the last element
      if (i == PLUS_REV) {
        break;
      }
    }
  ...
  public int compareTo(@NonNull GradleCoordinate that) {
    ...
    for (int i = 0; i < myRevisions.size(); ++i) {
      if (!myRevisions.get(i).equals(that.myRevisions.get(i))) {
        return myRevisions.get(i) - that.myRevisions.get(i);
      }
    }
    return 0;
  }
