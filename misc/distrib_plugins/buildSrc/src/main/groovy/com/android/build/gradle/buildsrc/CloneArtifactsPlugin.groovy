...
class CloneArtifactsPlugin implements org.gradle.api.Plugin<Project> {
    ...
    void apply(Project project) {
        ...
        if (project.rootProject == project) {
            def extension = project.extensions.create('cloneArtifacts', RootCloneArtifactsExtension)

            DownloadArtifactsTask downloadArtifactsTask = project.tasks.add("downloadArtifacts", DownloadArtifactsTask)
            downloadArtifactsTask.project = project
            downloadArtifactsTask.conventionMapping.mainRepo =  { project.file(extension.mainRepo) }
            downloadArtifactsTask.conventionMapping.secondaryRepo =  { project.file(extension.secondaryRepo) }
            ...
            project.extensions.create('cloneArtifacts', SubCloneArtifactsExtension)
