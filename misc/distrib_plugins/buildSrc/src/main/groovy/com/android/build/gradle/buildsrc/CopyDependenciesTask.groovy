        ...
        for (ResolvedArtifact artifact : artifacts) {
            ...
            if (!artifact.moduleVersion.id.group.startsWith("com.android") &&
                ...
                if (artifact.type == "jar") {
                    System.out.println("COPY: " + artifact.moduleVersion.id.toString() + " -> " + artifact.file.name)
                    Files.copy(artifact.file, new File(dir, artifact.file.name))
                } else {
                    System.out.println("IGNR(1): " + artifact.moduleVersion.id.toString())
                }
            } else {
                System.out.println("IGNR(0): " + artifact.moduleVersion.id.toString())
            }
        }
    }
}
