...
class CloneArtifactsTask extends DefaultTask {
    ...
    public void cloneArtifacts() {
        ...
        buildArtifactList(resolutionResult.getRoot(), mainList, null)

        resolutionResult = project.configurations.testCompile.getIncoming().getResolutionResult()
        buildArtifactList(resolutionResult.getRoot(), secondaryList, mainList)

        // manually add some artifacts that aren't detected because they are added dynamically
        // when their task run.
        try {
            resolutionResult = project.configurations.getByName("cloneArtifact").getIncoming().getResolutionResult()
            buildArtifactList(resolutionResult.getRoot(), secondaryList, mainList)
        ...
        for (ModuleVersionIdentifier id : secondaryList) {
            ...
            pullArtifact(id, getSecondaryRepo())
        }
    }

    private void buildArtifactList(ResolvedModuleVersionResult module,
                                   Set<ModuleVersionIdentifier> list,
                                   Set<ModuleVersionIdentifier> referenceList) {

        if (referenceList == null || !referenceList.contains(module.id)) {
            list.add(module.id)
        }
        ...
        for (DependencyResult d : module.getDependencies()) {
            if (d instanceof UnresolvedDependencyResult) {
                ...
                if (referenceList == null || !referenceList.contains(id)) {
                    list.add(id)
                }
            } else {
                buildArtifactList(((ResolvedDependencyResult) d).getSelected(), list, referenceList)
            }
        }
    }
    ...
    private boolean handlePom(File pomFile, File rootDestination) {
        ...
        if (relocation != null) {
            pullArtifact(relocation, rootDestination);
            return false
        }
