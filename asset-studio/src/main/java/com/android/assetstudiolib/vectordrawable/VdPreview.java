...
public class VdPreview {
    ...
    public static final String ANDROID_AUTO_MIRRORED = "android:autoMirrored";
     ...
     * @return the overridden XML file in one string.
     */
    @NonNull
    public static String overrideXmlContent(@NonNull String xmlFileContent,
        ...
        // TODO: update the width comparison b/t the originalWidth Vs. OverriedWidth()
        ...
        // update attributes, width and height always exist.
        ...
        if (info.needsOverrideWidth()) {
            ...
            if (srcSize != null) {
                srcSize.mVectorWidth = originalValue;
            }
        ...
        if (info.needsOverrideHeight()) {
            ...
            int originalValue = overrideDimension(overrideValue, nodeAttr);
            if (srcSize != null) {
                srcSize.mVectorHeight = originalValue;
            }
            if (originalValue != overrideValue) {
                isXmlFileContentChanged = true;
            }
        ...
        if (info.needsOverridAutoMirrored()) {
    ...
    public static BufferedImage getPreviewFromVectorXml(@NonNull TargetSize targetSize,
        ...
        if (forceImageSize > 0) {
            ...
            // So one of the dimension will be exactly forceImageSize.
            float scaledWidth = ratioToForceImageSize * vdWidth;
            float scaledHeight = ratioToForceImageSize * vdHeight;
            // At this point, image aspect ratio is still kept.
            // However, the absolute size may be too big, so we clamp it.
            // After clamping, the maximum aspect ratio supprted here is
