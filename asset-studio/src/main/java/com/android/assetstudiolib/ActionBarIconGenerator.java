...
public class ActionBarIconGenerator extends GraphicGenerator {
    ...
    public BufferedImage generate(GraphicGeneratorContext context, Options options) {
        ...
        if (actionBarOptions.density == Density.VECTOR) {
            Util.drawEffects(g, tempImage, 0, 0, new Effect[]{});
        }
        else if (actionBarOptions.theme == Theme.CUSTOM) {
            Util.drawEffects(g, tempImage, 0, 0, new Effect[]{new FillEffect(new Color(actionBarOptions.customThemeColor), 0.8),});
        }
        else if (actionBarOptions.theme == Theme.HOLO_LIGHT) {
            Util.drawEffects(g, tempImage, 0, 0, new Effect[]{new FillEffect(new Color(0x333333), 0.6),});
        }
        else if (actionBarOptions.theme == Theme.HOLO_DARK) {
            Util.drawEffects(g, tempImage, 0, 0, new Effect[]{new FillEffect(new Color(0xFFFFFF), 0.8)});
        }
