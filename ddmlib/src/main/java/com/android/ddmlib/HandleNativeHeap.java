...
final class HandleNativeHeap extends ChunkHandler {
    ...
    private void handleNHGT(Client client, ByteBuffer data) {
        ...
        //   };
        //
        // Only version 2 of this structure is supported.
        //
        // If the signature doesn't match, assume no special header and that
        // everything is being generated by a 32 bit system.
        ...
        // The structure of the rest of the header:
        //   size_t mapSize;
        //   size_t allocSize;
        ...
        if (mapSize > 0) {
            byte[] maps = new byte[(int)mapSize];
            data.get(maps, 0, mapSize);
            parseMaps(clientData, maps);
        }

        int iterations = (int)allocSize / (int)allocInfoSize;
    ...
    private void parseMaps(ClientData clientData, byte[] maps) {
        ...
        try {
            while ((line = reader.readLine()) != null) {
                ...
                // Assume that any library that starts with a /
                // is a shared library or executable that we will
                // try to symbolize.
