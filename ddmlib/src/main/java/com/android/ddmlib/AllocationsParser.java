...
public class AllocationsParser {
  private final File myAllocationsFile;
  private AllocationInfo[] myAllocations;

  public AllocationsParser(@NonNull File allocationsFile) {
    if (!allocationsFile.exists()) {
      throw new IllegalArgumentException(
        "Allocations file " + allocationsFile.getAbsolutePath() + " does not exist.");
    }
    myAllocationsFile = allocationsFile;
  }

  private static ByteBuffer mapFile(@NonNull File f) throws IOException {
  ...
  */
  public void parse() throws IOException {
    myAllocations = parseData(mapFile(myAllocationsFile));
  }

  public static AllocationInfo[] parseData(ByteBuffer data) {
  ...
  /*
   * Gets an array of AllocationInfo objects
   * {@link #parse()} should be called first to populate the array
   */
  @Nullable
