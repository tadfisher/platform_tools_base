...
final class Device implements IDevice {
    ...
    private static final long GET_PROP_TIMEOUT_MS = 2000;
    ...
    private String constructName() {
        if (isEmulator()) {
            ...
            try {
                String s = getSystemProperty(PROP_DEVICE_MANUFACTURER)
                        .get(GET_PROP_TIMEOUT_MS, TimeUnit.MILLISECONDS);
                manufacturer = cleanupStringForDisplay(s);

                s = getSystemProperty(PROP_DEVICE_MODEL)
                        .get(GET_PROP_TIMEOUT_MS, TimeUnit.MILLISECONDS);
                model = cleanupStringForDisplay(s);
            } catch (Exception e) {
    ...
    public boolean supportsFeature(@NonNull HardwareFeature feature) {
        if (mHardwareCharacteristics == null) {
            try {
                String characteristics = getSystemProperty(PROP_BUILD_CHARACTERISTICS)
                        .get(GET_PROP_TIMEOUT_MS, TimeUnit.MILLISECONDS);
    ...
    private int getApiLevel() {
        ...
        try {
            String apiLevel = getSystemProperty(PROP_BUILD_API_LEVEL)
                    .get(GET_PROP_TIMEOUT_MS, TimeUnit.MILLISECONDS);
