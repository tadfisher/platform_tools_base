...
public class AidlProcessor implements SourceSearcher.SourceFileProcessor {
    ...
    public void processFile(@NonNull File sourceFolder, @NonNull File sourceFile)
        ...
        System.out.println(">>> " + data.getMainFile());
        System.out.println("\t" + mParcelableOutputDir);
        System.out.println("\t" + data);
        if (data != null) {
            System.out.println("\t" + data.getOutputFiles());
        }

        if (mParcelableOutputDir != null && data != null && data.getOutputFiles().isEmpty()) {
            // looks like a parcelable. Store it in the
            // Make relative path from source folder to file.
            String folder = sourceFolder.getPath();
            String file = sourceFile.getPath();
            String relative = file.substring(folder.length());
