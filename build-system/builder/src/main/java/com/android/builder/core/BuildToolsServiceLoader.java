...
public enum BuildToolsServiceLoader {
    ...
    public synchronized BuildToolServiceLoader forVersion(BuildToolInfo buildToolInfo) {
        ...
        LoadedBuildTool loadedBuildTool = findVersion(buildToolInfo.getRevision());
        if (loadedBuildTool == null) {
            loadedBuildTool = new LoadedBuildTool(buildToolInfo.getRevision(),
    ...
    public static final class BuildToolServiceLoader {
        ...
        public synchronized  <T> ServiceLoader<T> getServiceLoader(Service<T> serviceType)
            ...
            ServiceLoader<T> serviceLoader = getLoadedServiceLoader(serviceType.getServiceClass());
            ...
            serviceLoader = ServiceLoader.load(serviceType.getServiceClass(), cl);
        ...
        /**
         * Return the first service instance for the requested service type or null of none exist.
         * @param serviceType the requested service type encapsulation.
        ...
        private <T> ServiceLoader<T> getLoadedServiceLoader(Class<T> serviceType) {
            for (LoadedServiceLoader<?> loadedServiceLoader : loadedServicesLoaders) {
                if (loadedServiceLoader.serviceType.equals(serviceType)) {
                    return (ServiceLoader<T>) loadedServiceLoader.serviceLoader;
                }
            }
            return null;
        }
