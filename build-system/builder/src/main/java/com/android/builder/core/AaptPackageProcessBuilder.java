...
public class AaptPackageProcessBuilder extends ProcessEnvBuilder<AaptPackageProcessBuilder> {
     ...
     * @return itself
     */
    public AaptPackageProcessBuilder setType(@NonNull VariantConfiguration.Type type) {
        this.mType = type;
        return this;
    }
    ...
    public ProcessInfo build(
        ...
        if (mPseudoLocalesEnabled) {
            if (buildToolInfo.getRevision().getMajor() >= 21) {
                builder.addArg("--pseudo-localize");
            } else {
        ...
        if (mType == VariantConfiguration.Type.LIBRARY) {
            builder.addArg("--non-constant-id");
        }
        ...
        if (mOptions.getFailOnMissingConfigEntry()) {
            if (buildToolInfo.getRevision().getMajor() > 20) {
                builder.addArg("--error-on-missing-config-entry");
            } else {
