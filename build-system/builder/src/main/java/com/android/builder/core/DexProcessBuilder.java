...
public class DexProcessBuilder extends ProcessEnvBuilder<DexProcessBuilder> {
    ...
    public JavaProcessInfo build(
        ...
        if (mVerbose) {
            builder.addArg("--verbose");
        }

        if (dexOptions.getJumboMode()) {
            builder.addArg("--force-jumbo");
        }

        if (mIncremental) {
            builder.addArg("--incremental");
            builder.addArg("--no-strict");
        }

        if (mNoOptimize) {
            builder.addArg("--no-optimize");
        }
        ...
        if (mMultiDex) {
            builder.addArg("--multi-dex");

            if (mMainDexList != null ) {
                builder.addArg("--main-dex-list");
                builder.addArg(mMainDexList.getAbsolutePath());
            }
        }
        ...
        builder.addArg(mOutputFile.getAbsolutePath());
