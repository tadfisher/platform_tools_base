...
public class ConnectedDeviceProvider extends DeviceProvider {
    ...
    public void init() throws DeviceException {
        ...
        if (filteredDevices.isEmpty()) {
            throw new DeviceException("Connected device with serial %s not found!", androidSerial);
        }
        ...
        if (localDevices.isEmpty()) {
            if (isValidSerial) {
                throw new DeviceException("Connected device with serial $1%s is not online.",
                        androidSerial);
            } else {
