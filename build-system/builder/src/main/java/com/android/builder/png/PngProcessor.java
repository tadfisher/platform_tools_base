...
public class PngProcessor {
    ...
    private void processImageContent(@NonNull BufferedImage image) throws NinePatchException,
        ...
        ColorType colorType = createImage(content, width, startX, endX, startY, endY);
    ...
    private ColorType createImage(@NonNull int[] content, int scanline,
            int startX, int endX, int startY, int endY) throws IOException {
        ...
        // for now remove grayscale encoding
        isGrayscale = false;
        ...
        if (colorType == ColorType.PLTE) {
            ...
            for (int idx = 0; idx < paletteColorCount; idx++) {
                ...
                rgbPalette[idx * 3]     = (byte) ((color >> 24) & 0xFF);
    ...
    private void processBorder(int[] content, int width, int height)
        ...
        addChunk(ninePatchChunk.getChunk());
        if (layoutBoundChunk != null) {
            addChunk(layoutBoundChunk.getChunk());
        }
    }
