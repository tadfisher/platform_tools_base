    ...
    private void handleAppinviteService(JsonObject clientObject, Map<String, String> resValues) throws IOException {
        ...
        if (otherPlatformOauthClientArray != null) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (otherPlatformOauthClientElement == null || !otherPlatformOauthClientElement.isJsonObject()) continue;

                JsonObject otherPlatformOauthClientObject = otherPlatformOauthClientElement.getAsJsonObject();

                JsonPrimitive clientType = otherPlatformOauthClientObject.getAsJsonPrimitive("client_type");
                if (clientType == null) continue;

                String clientTypeStr = clientType.getAsString();
                if (!OAUTH_CLIENT_TYPE_IOS.equals(clientTypeStr)) continue;

                JsonPrimitive id = otherPlatformOauthClientObject.getAsJsonPrimitive("client_id");
                if (id == null) continue;
