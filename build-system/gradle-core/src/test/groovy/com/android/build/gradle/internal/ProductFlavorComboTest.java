...
public class ProductFlavorComboTest {
    ...
    @Mock
    private GradleGroupableProductFlavor f1;
    @Mock
    private GradleGroupableProductFlavor f2;
    @Mock
    private GradleGroupableProductFlavor f1d1;
    @Mock
    private GradleGroupableProductFlavor f2d1;
    @Mock
    private GradleGroupableProductFlavor f1d2;
    @Mock
    private GradleGroupableProductFlavor f2d2;

    @Before
    public void setup() {
        MockitoAnnotations.initMocks(this);
        Mockito.when(f1.getName()).thenReturn("flavor1");
        Mockito.when(f1.getFlavorDimension()).thenReturn(null);
        Mockito.when(f2.getName()).thenReturn("flavor2");
        Mockito.when(f2.getFlavorDimension()).thenReturn(null);

        Mockito.when(f1d1.getName()).thenReturn("flavor1");
        Mockito.when(f1d1.getFlavorDimension()).thenReturn(DIMENSION1);
        Mockito.when(f2d1.getName()).thenReturn("flavor2");
        Mockito.when(f2d1.getFlavorDimension()).thenReturn(DIMENSION1);

        Mockito.when(f1d2.getName()).thenReturn("flavor1");
        Mockito.when(f1d2.getFlavorDimension()).thenReturn(DIMENSION2);
        Mockito.when(f2d2.getName()).thenReturn("flavor2");
        Mockito.when(f2d2.getFlavorDimension()).thenReturn(DIMENSION2);
    }
    ...
    public void createGroupListEmpty() throws Exception {
        assertEqualGroupList(
                ...
                ProductFlavorCombo.createCombinations(
                        ...
                        Collections.<GradleGroupableProductFlavor>emptyList()));
    }
