...
public class RemoteTestRunnerTask extends BaseTask implements AndroidTestTask {
    ...
    protected void runTests() {
        ...
        if (!testsFound()) {
            ...
            Map<String, String[]> testResults = remoteTestRunner.runTests(
                new TestDataImpl(testVariantData),
                testVariantData.outputs.get(0).getOutputFile()
            )
            for (Map.Entry<String, String[]> testResult: testResults) {

                def parser = new InstrumentationResultParser(testResults.key, runListener);
                parser.processNewLines(testResult.value)
                success = true
                parser.flush()
            }
            remoteTestRunner.terminate()
        }
