...
public class DeviceProviderInstrumentTestTask extends BaseTask implements AndroidTestTask {
    ...
    protected void runTests() throws DeviceException, IOException, InterruptedException,
        ...
        if (!testsFound()) {
            ...
            try {
                success = ((SimpleTestRunner) testRunner)
                        .runTests(getProject().getName(), flavor, testApk, testData,
                                deviceProvider.getDevices(), deviceProvider.getMaxThreads(),
                                deviceProvider.getTimeoutInMs(), extraArgs, resultsOutDir,
                                coverageOutDir, getILogger());
            } finally {
        ...
        if (!success) {
            ...
            String reportUrl = new ConsoleRenderer()
                    .asClickableFileUrl(new File(reportOutDir, "index.html"));
