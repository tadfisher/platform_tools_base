...
public class DeviceProviderInstrumentTestTask extends BaseTask implements AndroidTestTask {
    ...
    public static class ConfigAction implements TaskConfigAction<DeviceProviderInstrumentTestTask> {
        ...
        public ConfigAction(VariantScope scope, DeviceProvider deviceProvider) {
            this.scope = scope;
            this.deviceProvider = deviceProvider;
        }
        ...
        public void execute(DeviceProviderInstrumentTestTask task) {
            ...
            if (connected) {
                task.setDescription("Installs and runs the tests for " +
                        scope.getTestedVariantData().getName() + " on connected devices.");
            } else {
                task.setDescription("Installs and runs the tests for " +
                        scope.getTestedVariantData().getName() + " using provider: " +
                        StringHelper.capitalize(deviceProvider.getName()));

            }
            task.setGroup(JavaBasePlugin.VERIFICATION_GROUP);
            task.setAndroidBuilder(scope.getGlobalScope().getAndroidBuilder());
            TestVariantData testVariantData = (TestVariantData) scope.getVariantData();
            final TestData testData = new TestDataImpl(testVariantData);
            ...
            ConventionMappingHelper.map(task, "adbExec", new Closure<File>(this, this) {
            ...
            ConventionMappingHelper.map(task, "coverageDir", new Callable<File>() {
                @Override
                public File call() {
