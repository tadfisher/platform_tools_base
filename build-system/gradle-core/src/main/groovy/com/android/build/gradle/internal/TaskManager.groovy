...
abstract class TaskManager {
    ...
    public void createMockableJarTask() {
        ...
        conventionMapping(createMockableJar).map("outputFile") {
            project.file(
                    "$project.buildDir/${AndroidProject.FD_INTERMEDIATES}/mockable-${sdkName}.jar")
        }
...
//                GenerateBuildConfig)
    ...
    public void createJackTask(
        ...
        conventionMapping(jillRuntimeTask).map("outputFolder") {
            project.file(
                    "${project.buildDir}/${AndroidProject.FD_INTERMEDIATES}/jill/${config.dirName}/runtime")
        }
        ...
        conventionMapping(jillPackagedTask).map("outputFolder") {
            project.file(
                    "${project.buildDir}/${AndroidProject.FD_INTERMEDIATES}/jill/${config.dirName}/packaged")
        }
        ...
        conventionMapping(compileTask).map("destinationDir") {
            project.file("$project.buildDir/${AndroidProject.FD_INTERMEDIATES}/dex/${config.dirName}")
        }

        conventionMapping(compileTask).map("jackFile") {
            project.file(
                    "$project.buildDir/${AndroidProject.FD_INTERMEDIATES}/packaged/${config.dirName}/classes.zip")
        }

        conventionMapping(compileTask).map("tempFolder") {
            project.file("$project.buildDir/${AndroidProject.FD_INTERMEDIATES}/tmp/jack/${config.dirName}")
        }
...
//        tasks.create("assemble${variantData.variantConfiguration.fullName.capitalize()}") { Task assembleTask ->
...
//        }
    ...
    public Copy getJacocoAgentTask() {
        if (jacocoAgentTask == null) {
            ...
            jacocoAgentTask.into "$project.buildDir/${AndroidProject.FD_INTERMEDIATES}/jacoco"
        }
...
//        scope.preBuildTask = androidTasks.create(tasks,
//                "pre${variantData.variantConfiguration.fullName.capitalize()}Build") { Task preBuildTask ->
//            variantData.preBuildTask = preBuildTask
