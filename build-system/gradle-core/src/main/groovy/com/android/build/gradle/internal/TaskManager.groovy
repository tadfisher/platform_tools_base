...
abstract class TaskManager {
    ...
    /**
     * Makes the task created by {@link #createJavacTask(TaskFactory, VariantScope)} the one used by
     * the top-level "compile" task. Only called if Jack is not used.
     */
    public void useJavacTask(
            @NonNull TaskFactory tasks,
            @NonNull VariantScope scope) {
        scope.compileTask.dependsOn(tasks, scope.javacTask);
        scope.activeJavaCompileTask = scope.javacTask
        scope.variantData.activeJavaCompileTask = scope.variantData.javacTask
    }
    ...
    protected void createConnectedTestForVariantData(
        ...
        if (baseVariantData.variantConfiguration.buildType.isTestCoverageEnabled()) {
            ...
            conventionMapping(reportTask).map("classDir") {
                // DO NOT SUBMIT: Do we support coverage with Jack?
    ...
    public void createJackTask(
        ...
        variantData.activeJavaCompileTask = jackTask
    ...
    public void createPackagingTask(
        ...
        for (ApkVariantOutputData vod : variantData.outputs) {
            ...
            packageApp.optionalDependsOn(
                    ...
                    variantOutputScope.variantScope.activeJavaCompileTask,
                    variantData.activeJavaCompileTask,  // TODO: Remove when Jack is converted to AndroidTask.
                    variantOutputData.packageSplitResourcesTask,
                    variantOutputData.packageSplitAbiTask);
