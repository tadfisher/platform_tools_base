...
public class MergeJavaResourcesTask extends DefaultAndroidTask implements JavaResourcesProvider {
    ...
    public interface Filter {
        ...
        @NonNull
        Action apply(@NonNull String input);
    }
     ...
     */
    private static final class FileFilter implements Filter, SignedJarBuilder.IZipEntryFilter {
        @Nullable
        ...
        @NonNull
        @Override
        public Action apply(@NonNull String input) {
            //noinspection VariableNotUsedInsideIf
            if (packagingOptions != null) {
                if (excludes.contains(input)) {
                    return Action.IGNORE;
                }

                if (pickFirsts.contains(input)) {
                    if (usedPickFirsts == null) {
                        usedPickFirsts = Sets.newHashSetWithExpectedSize(pickFirsts.size());
                    }

                    if (usedPickFirsts.contains(input)) {
                        return Action.IGNORE;
                    } else {
                        usedPickFirsts.add(input);
                    }
                }

                if (packagingOptions.getMerges().contains(input)) {
                    return Action.MERGE;
                }
            }
            return Action.COPY;
