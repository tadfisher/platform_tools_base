...
public class ManifestMerger2 {
    ...
    private MergingReport merge() throws MergeFailureException {
        ...
        // first do we have a package declaration or injection.
        Optional<XmlAttribute> mainPackageAttribute =
                loadedMainManifestInfo.getXmlDocument().getPackage();

        // this should have been enforced during PreValidation step.
        Preconditions.checkState(mainPackageAttribute.isPresent());
                ...
                mergingReportBuilder.addMessage(
                        ...
                        String.format(
                                "Overlay manifest:package attribute declared at %1$s value=(%2$s)\n"
                                        ...
                                        + "\tSuggestion : remove the overlay AndroidManifest.xml "
                                        + "declaration and place it in the build.gradle:\n"
                                        + "\t\tflavorName {\n"
                                        + "\t\t\tapplicationId = \"%2$s\"\n"
                                        + "\t\t}",
