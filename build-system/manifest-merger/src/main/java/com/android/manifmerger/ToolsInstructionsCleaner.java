    ...
    private static MergingReport.Result cleanToolsReferences(
            ...
            for (int i = 0; i < namedNodeMap.getLength(); i++) {
                ...
                if (SdkConstants.TOOLS_URI.equals(attribute.getNamespaceURI())) {
                    ...
                    if (attribute.getLocalName().equals(XmlElement.TOOLS_NODE_LOCAL_NAME)
                            && attribute.getNodeValue().equals(XmlUtils.constantNameToCamelCase(
                                    NodeOperationType.REMOVE.toString()))) {
        ...
        for (int i = 0; i < childNodes.getLength(); i++) {
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE) {
                if (cleanToolsReferences((Element) node, logger) == MergingReport.Result.ERROR) {
                    return MergingReport.Result.ERROR;
                }
            }
        }
