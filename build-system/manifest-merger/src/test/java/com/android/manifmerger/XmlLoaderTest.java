...
public class XmlLoaderTest extends TestCase {
    ...
    public void testAndroidPrefix() throws IOException, SAXException, ParserConfigurationException {
        ...
        Optional<XmlNode> applicationOptional = xmlDocument
                .getNodeByTypeAndKey(XmlNodeTypes.APPLICATION, null);
        assertTrue(applicationOptional.isPresent());
        Node label = applicationOptional.get().getElement().getAttributes().item(0);
    ...
    public void testToolsPrefix() throws IOException, SAXException, ParserConfigurationException {
        ...
        Optional<XmlNode> applicationOptional = xmlDocument
                .getNodeByTypeAndKey(XmlNodeTypes.APPLICATION, null);
        assertTrue(applicationOptional.isPresent());
        Element application = applicationOptional.get().getElement();
        ...
        assertEquals("android:label", label.getNodeName());
        Node tools = application.getAttributeNodeNS(XmlLoader.TOOLS_URI, "node");
        assertEquals("replace", tools.getNodeValue());

        // check positions.
        PositionXmlParser.Position applicationPosition = xmlDocument.getPositionXmlParser()
                .getPosition(application);
        assertNotNull(applicationPosition);
        assertEquals(5, applicationPosition.getLine());
        assertEquals(4, applicationPosition.getColumn());

        PositionXmlParser.Position toolsPosition = xmlDocument.getPositionXmlParser().getPosition(tools);
        assertNotNull(toolsPosition);
        assertEquals(5, toolsPosition.getLine());
        assertEquals(50, toolsPosition.getColumn());
    }
    ...
    private XmlDocument xmlDocumentFromString(String input)
