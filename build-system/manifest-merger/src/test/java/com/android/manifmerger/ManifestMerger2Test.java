...
public class ManifestMerger2Test extends ManifestMergerTest {
    ...
    /**
     * Processes the data from the given {@link TestFiles} by invoking {@link
    ...
    void processTestFiles(TestFiles testFiles) throws Exception {
        ...
        MergingReport mergeReport = ManifestMerger2.newBuilder(testFiles.getMain())
                .addLibraryManifests(testFiles.getLibs())
                .merge(byteArrayOutputStream);

        mergeReport.log(Logger.getAnonymousLogger());
        ...
        MergingReport.Builder comparingReport = new MergingReport.Builder();
        assertTrue(expectedResult.compareXml(actualResult, comparingReport));
