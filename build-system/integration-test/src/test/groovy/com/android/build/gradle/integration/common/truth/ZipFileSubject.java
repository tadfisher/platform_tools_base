...
public class ZipFileSubject extends Subject<ZipFileSubject, File> {
     ...
     */
    public void containsEntry(String path, String content) {
        assertThat(extractContentAsString(path).trim()).comparesEqualTo(content.trim());
    }

    /**
     * Asserts the zip file contains a file with the specified byte array content.
     */
    public void containsEntry(String path, byte[] content) {
        assertThat(extractContentAsByte(path)).isEqualTo(content);
    }
