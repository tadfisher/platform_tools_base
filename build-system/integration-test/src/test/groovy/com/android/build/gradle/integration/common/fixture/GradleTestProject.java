...
public class GradleTestProject implements TestRule {
     ...
     * Runs gradle on the project, and returns the project model.  Throws exception on failure.
     *
     * @param emulateOlderIde whether to emulate an older IDE (studio 1.0) querying the model.
     * @param tasks Variadic list of tasks to execute.
     *
     * @return the AndroidProject model for the project.
     */
    @NonNull
    public AndroidProject executeAndReturnModel(boolean emulateOlderIde, String ... tasks) {
        //noinspection ConstantConditions
        return execute(Collections.<String>emptyList(), true, emulateOlderIde, tasks);
    }
     ...
     */
    @NonNull
    public Map<String, AndroidProject> executeAndReturnMultiModel(boolean emulateOlderIde, String ... tasks) {
        ProjectConnection connection = getProjectConnection();
        try {
            executeBuild(Collections.<String>emptyList(), connection, tasks);

            return buildModel(connection, emulateOlderIde);
     ...
     */
    @NonNull
    public AndroidProject getSingleModel(boolean emulateOlderIde) {
        ProjectConnection connection = getProjectConnection();
        try {
            Map<String, AndroidProject> modelMap = buildModel(connection, emulateOlderIde);
     ...
     */
    @NonNull
    public Map<String, AndroidProject> getAllModels(boolean emulateOlderIde) {
        ProjectConnection connection = getProjectConnection();
        try {
            return buildModel(connection, emulateOlderIde);
