...
public class GradleTestProject implements TestRule {
    ...
    public String getGradleBuildscript() {
        ...
        File buildScript = new File(Builder.TEST_PROJECT_DIR, buildScriptName);
        File commonHeader = new File(Builder.TEST_PROJECT_DIR, COMMON_HEADER);
        try {
            return "/* Generated from " + buildScript.getName() +
                    " and " + commonHeader.getName() + ". */\n" +
                    Files.toString(commonHeader, Charsets.UTF_8) + "\n" +
                    "buildscript {\n" +
                    Files.toString(buildScript, Charsets.UTF_8) + "\n" +
                    "}\n";
        } catch (IOException e) {
