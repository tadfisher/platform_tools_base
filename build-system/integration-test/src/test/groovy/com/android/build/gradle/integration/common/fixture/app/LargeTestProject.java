...
public class LargeTestProject implements TestProject {
    ...
    interface ModuleFactory {
        GradleModule createModule(File location, String path, List<? extends GradleModule> projectDeps);
    }

    static class AndroidModuleFactory implements ModuleFactory {
        @Override
        public GradleModule createModule(File location, String path, List<? extends GradleModule> projectDeps) {
    ...
    public LargeTestProject(
            ProjectType type, int maxDepth, int maxBreadth) {
        this.type = type;
    ...
    private GradleModule createProject(
        ...
        if (depth < maxDepth) {
            for (int i = 0 ; i < maxBreadth ; i++) {
                ...
                GradleModule project = createProject(factory, depth + 1, path + "lib"+newSuffix+":", new File(location, "lib" + newSuffix), newSuffix);
                deps.add(project);
            }
        }
    ...
    private static void createSettingsGradle(GradleModule project, File location) throws IOException {
    ...
    private static void buildSettingsGradle(GradleModule project, StringBuilder builder) {
        builder.append("include '").append(project.getPath()).append("'\n");
        for (GradleModule dep : project.getProjectDeps()) {
            buildSettingsGradle(dep, builder);
        }
    }

    private static void createGradleProperties(File location) throws IOException {
        Files.write(
