...
class NdkPlugin implements Plugin<Project> {
        ...
        configurationActions.add(Actions.filter(
                ...
                new Spec<Project>() {
                    ...
                    boolean isSatisfiedBy(Project p) {
                        ...
                        return androidPlugin.extension.useNewNdkPlugin &&
    ...
    public Iterable<Task> getNdkTasks(VariantConfiguration variantConfig) {
        ...
        binaries.collect { ProjectSharedLibraryBinary binary ->
            // TODO: Gradle 2.0 will provide a method to get the lifecycle task directly from the
            // binary.
    ...
    private void hideUnwantedTasks() {
        ...
        project.libraries.getByName(ndkExtension.getModuleName()) {
            Iterable<Task> lifecycleTasks =
                    binaries.withType(ProjectStaticLibraryBinary.class).collect {
                        ProjectStaticLibraryBinary binary ->
                            project.tasks.getByName(binary.getNamingScheme().getLifecycleTaskName())
                    }
