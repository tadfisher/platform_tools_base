...
public class ResourceUsageAnalyzer {
    ...
    private void recordMapping(@Nullable File mapping) {
    ...
    private void recordResourcesUsages(@NonNull File file, @Nullable Resource from) {
        try {
            ...
            System.out.println("Couldn't parse " + file.getPath());
            e.printStackTrace();
        }
    }
    ...
    private void recordUsages(File jarFile) {
        if (!jarFile.exists()) {
            return;
        }
    ...
    private void gatherResourceValues(File file) {
        if (file.isDirectory()) {
    ...
    private void parseResourceClass(File file) {
        try {
