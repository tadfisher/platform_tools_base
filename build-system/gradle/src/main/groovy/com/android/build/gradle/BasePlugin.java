...
public abstract class BasePlugin {
    ...
    private void verifyRetirementAge() {
        ...
        int retirementAge =
                getRetirementAge(manifest.getMainAttributes().getValue("Plugin-Version"));
        ...
        if (days > retirementAge) {
            ...
            if (dailyOverride == null) {
                String message = "Plugin is too old, please update to a more recent version, " +
    ...
    private class UnsupportedAction implements Action<Object> {
    ...
    private void checkModulesForErrors() {
        ...
        for (Project subProject : rootProject.getAllprojects()) {
            ...
            if (subProjectsById.containsKey(id)) {
                String message = "Your project contains 2 or more modules with the same " +
                        "identification  " + id + " at " + subProjectsById.get(id).getPath() +
                        " and " + subProject.getPath() +". You must use different identification " +
                        "(either name or group) for each modules.";
