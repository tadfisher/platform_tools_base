...
public class DeviceProviderInstrumentTestTask extends BaseTask implements AndroidTestTask {
    ...
    /**
     * Determines if there are any tests to run. If yes, and the test runner returns with no
     * results, we fail (since this is most likely a problem with the device setup). If no, the
     * task will succeed.
     *
    ...
    private boolean testsFound() {
        ...
        Collection<File> testDirectories =
                variantConfiguration.sortedSourceProviders*.javaDirectories.flatten()

        testDirectories*.exists().any()
    }
}
