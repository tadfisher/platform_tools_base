    ...
    public ApplicationVariantData createVariantData(
        ...
        if (variant.getSplitHandlingPolicy() ==
            ...
            for (String density : densities) {
                for (String abi : abis) {
                    ImmutableList.Builder<SplitData> builder = ImmutableList.builder();
                    if (density != null) {
                        builder.add(new SplitData(SplitOutput.FilterType.DENSITY, density));
                    }
                    if (abi != null) {
                        builder.add(new SplitData(SplitOutput.FilterType.ABI, abi));
                    }
                    variant.createOutput(
                            SplitOutput.OutputType.FULL_SPLIT,
                            builder.build());
                }
            }
        } else {
            variant.createOutput(SplitOutput.OutputType.MAIN,
