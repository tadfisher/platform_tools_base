...
public abstract class BaseVariantOutputData implements SplitOutput {
    ...
    public abstract ImmutableList<SplitApkOutput> getOutputSplitFiles();
    ...
    public String getDirName() {
        ...
        return variantData.getVariantConfiguration().computeDirNameWithSplits(
                getFilter(SplitOutput.DENSITY),
                getFilter(SplitOutput.ABI));
    }
    ...
    private String getFilterName() {
        ...
        String densityFilter = getFilter(SplitOutput.DENSITY);
        if (densityFilter != null) {
            sb.append(densityFilter);
        }
        String abiFilter = getFilter(SplitOutput.ABI);
        if (abiFilter != null) {
            if (sb.length() > 0) {
