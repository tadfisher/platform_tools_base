...
/**
 * Transformer which inserts annotations into the android.jar stubs library
 */
@SuppressWarnings({"SpellCheckingInspection", "UnusedDeclaration"})
public class JarRewriter {
    @NonNull
    private final Extractor mExtractor;
    private int mInsertedParameterCount;
    private int mInsertedReturnValueCount;
    private int mInsertedFieldCount;
    ...
    private void transform(File source, File dest) throws IOException {
        ...
        try {
            ...
            String message = "Inserted annotations into " + source +
                    ...
                    mInsertedReturnValueCount + " method return value annotations inserted\n" +
                    mInsertedParameterCount + " method parameter annotations inserted\n" +
                    mInsertedFieldCount + " field annotations inserted\n";
    ...
    private class ClassVisitor extends org.objectweb.asm.ClassVisitor {
        private final ClassReader mClassReader;

        ClassVisitor(ClassWriter writer, ClassReader classReader) {
            super(ASM4, writer);
            mClassReader = classReader;
        }
        ...
        public FieldVisitor visitField(int access, String name, String desc, String signature,
            ...
            if (visitor != null) {
                String owner = mClassReader.getClassName();
                String fqn = JarRewriter.fromVmType(owner);
                String annotation = mExtractor.getFieldAnnotation(fqn, name);
                if (annotation != null) {
        ...
        public MethodVisitor visitMethod(int access, String name, String desc, String signature,
            ...
            if (visitor != null) {
                ...
                String annotation = mExtractor.getMethodAnnotation(fqn, methodName, returnType,
                        parameterList, isConstructor, -1);
                if (annotation != null) {
                    String vmName = 'L' + getJvmName(annotation) + ';';
                    org.objectweb.asm.AnnotationVisitor av = visitor.visitAnnotation(vmName, false);
                    av.visitEnd();
                    mInsertedReturnValueCount++;
                }

                // Parameter annotations
                int argCount = getArgCount(desc);
                for (int i = 0; i < argCount; i++) {
                    annotation = mExtractor.getMethodAnnotation(fqn, methodName, returnType,
                            parameterList, isConstructor, i);
                    if (annotation != null) {
                        String vmName = 'L' + getJvmName(annotation) + ';';
                        org.objectweb.asm.AnnotationVisitor av = visitor.visitParameterAnnotation(i, vmName, false);
                        av.visitEnd();
                        mInsertedParameterCount++;
                    }
                }
            }
            return visitor;
