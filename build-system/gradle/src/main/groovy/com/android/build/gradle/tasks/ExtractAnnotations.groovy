...
class ExtractAnnotations extends AbstractCompile {
    ...
    public static final String ANNOTATIONS_JAR_PREFIX = "support-annotations-"
    ...
    /**
     * A list of existing annotation zip files to merge in. This can be used to merge in
    ...
    public boolean hasAndroidAnnotations() {
        // TODO: Replace this with a check on the VariantDependencies object, which
        // will be initialized in BasePlugin#addDependency where we'll look for the
        // android support annotations dependency and if found store that fact on the
        // VariantDependencies object

        def config = variant.variantConfiguration
        for (LibraryDependency dependency : config.getAllLibraries()) {
            for (File jar : dependency.getLocalJars()) {
                if (dependency.jarFile.name.startsWith(ANNOTATIONS_JAR_PREFIX)) {
                    return true;
                }
            }
        }

        for (JarDependency dependency : config.getJars()) {
            if (dependency.jarFile.name.startsWith(ANNOTATIONS_JAR_PREFIX)) {
                return true;
            }
        }
