    ...
    public void execute(ProjectInternal project) {
        ...
        if (extension.getModuleName() == null || extension.getModuleName().equals("")) {
            throw new InvalidUserDataException(
                    "moduleName must be set for Android NDK plugin.")
        }

        if (extension.getCompileSdkVersion() == null) {
            // Retrieve compileSdkVersion from Android plugin if it is not set for the NDK plugin.
            BasePlugin androidPlugin = project.plugins.findPlugin(AppPlugin.class)
            if (androidPlugin == null) {
                androidPlugin = project.plugins.findPlugin(LibraryPlugin.class)
            }
            if (androidPlugin != null) {
