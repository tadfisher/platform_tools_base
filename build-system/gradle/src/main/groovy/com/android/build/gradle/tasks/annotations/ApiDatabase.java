...
public class ApiDatabase {
    ...
    @NonNull private final  Map<String,Set<String>> mFieldMap =
            Maps.newHashMapWithExpectedSize(1000);
    /** Map from class name to map of method names whose values are overloaded signatures */
    @NonNull private final  Map<String,Map<String,List<String>>> mMethodMap =
            Maps.newHashMapWithExpectedSize(1000);
    @NonNull private final Map<String, List<String>> mInheritsFrom =
            Maps.newHashMapWithExpectedSize(1000);
    ...
    public boolean hasMethod(String className, String methodName, String arguments) {
        Map<String, List<String>> methods = mMethodMap.get(className);
        if (methods != null) {
        ...
        List<String> inheritsFrom = mInheritsFrom.get(className);
    ...
    public boolean hasField(String className, String fieldName) {
        Set<String> fields = mFieldMap.get(className);
    ...
    private void readApi() {
        ...
        for (String line : mLines) {
            ...
            if (line.startsWith("method ")) {
                ...
                if (!matcher.matches()) {
                    System.out.println("Warning: Did not match as a member: " + line);
                } else {
                    assert currentClass != null;
                    Map<String,List<String>> memberMap = mMethodMap.get(currentClass);
                    if (memberMap == null) {
                        memberMap = Maps.newHashMap();
                        mMethodMap.put(currentClass, memberMap);
                    }
                ...
                if (!matcher.matches()) {
                    ...
                    Set<String> fieldSet = mFieldMap.get(currentClass);
                    if (fieldSet == null) {
                        fieldSet = Sets.newHashSet();
                        mFieldMap.put(currentClass, fieldSet);
                    }
                ...
                if (!matcher.matches()) {
                    System.out.println("Warning: Did not match as a package: " + line);
                } else {
                ...
                if (!matcher.matches()) {
                    System.out.println("Warning: Did not match as a class/interface: " + line);
                } else {
    ...
    private void addInheritsFrom(String cls, String inheritsFrom) {
        List<String> list = mInheritsFrom.get(cls);
        if (list == null) {
            list = Lists.newArrayList();
            mInheritsFrom.put(cls, list);
        }
