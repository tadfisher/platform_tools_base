...
class TaskManager {
    ...
    public void createRenderscriptTask(
        ...
        // only put plugin dependency if rs will generate Java code
        if (!ndkMode) {
    ...
    public MergeResources basicCreateMergeResourcesTask(
        ...
        conventionMapping(mergeResourcesTask).
                map("useNewCruncher") { extension.aaptOptions.useNewCruncher }
        ...
        for (BaseVariantOutputData vod : variantData.outputs) {
            ...
            if (variantData.getSplitHandlingPolicy() ==
                ...
                Set<String> filters = new HashSet<String>(
                        extension.getSplits().getDensityFilters());
                filters.addAll(extension.getSplits().getLanguageFilters());
    ...
    public void createSplitResourcesTasks(
        ...
        Set<String> densityFilters =
                removeAllNullEntries(extension.getSplits().getDensityFilters());
        Set<String> abiFilters = removeAllNullEntries(extension.getSplits().getAbiFilters());
        Set<String> languageFilters =
                removeAllNullEntries(extension.getSplits().getLanguageFilters());
    ...
    public void createSplitAbiTasks(
        ...
        VariantConfiguration config = variantData.variantConfiguration
        Set<String> filters = new HashSet<String>();
        for (String abi : extension.getSplits().getAbiFilters()) {
            if (abi != null) {
                filters.add(abi);
            }
        }
        ...
        conventionMapping(variantOutputData.packageSplitAbiTask).
                map("packagingOptions") { extension.packagingOptions }

        ((ApkVariantOutputData) variantOutputData).splitZipAlign.dependsOn variantOutputData.packageSplitAbiTask
    }

    /**
     * Calculate the list of folders that can contain jni artifacts for plugin variant.
    ...
    public void createCompileTask(
        ...
        // setup the boot classpath just before the task actually runs since plugin will
    ...
    public void createGenerateMicroApkDataTask(
        ...
        // the merge res task will need to run after plugin one.
    ...
    public void createAndroidTestVariantTasks(@NonNull TestVariantData variantData) {
        ...
        // get single output for now (though plugin may always be the case for tests).
        BaseVariantOutputData variantOutputData = variantData.outputs.get(0)
        ...
        if (testedVariantData.variantConfiguration.type == VariantConfiguration.Type.LIBRARY) {
            // in plugin case the tested library must be fully built before test can be built!
    ...
    private void createLintVitalTask(@NonNull ApkVariantData variantData) {
        ...
        if (!variantData.variantConfiguration.buildType.debuggable &&
            ...
            // TODO: Make plugin task depend on lintCompile too (resolve initialization order first)
    ...
    void createUnitTestTasks() {
        ...
        plugin.variantDataList.findAll { it.variantConfiguration.type == UNIT_TEST }.
                each { loopVariantData ->
    ...
    private static void fixTestTaskSources(@NonNull Test testTask) {
        ...
        // assumes all Test tasks are fully configured at plugin point), so we have to remove the
    ...
    public void createConnectedCheckTasks(boolean hasFlavors, boolean isLibraryTest) {
        ...
        // if more than one flavor, create a report aggregator task and make plugin the parent
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (baseVariantData instanceof TestedVariantData) {
                ...
                // create the check tasks for plugin test
        ...
        // To do plugin, we make the children tasks ignore their errors (ie they won't fail and
        // stop the build).
    ...
    public void createJackTask(
        ...
        conventionMapping(compileTask).
                map("javaMaxHeapSize") { extension.dexOptions.getJavaMaxHeapSize() }
        ...
        for (ApkVariantOutputData vod : variantData.outputs) {
            ...
            conventionMapping(packageApp).map("abiFilters") {
                if (variantOutputData.mainOutputFile.getFilter(OutputFile.ABI) != null) {
                    return ImmutableSet.
                            of(variantOutputData.mainOutputFile.getFilter(OutputFile.ABI))
                }
            ...
            conventionMapping(packageApp).map("outputFile") {
                // if plugin is the final task then the location is
            ...
            if (multiOutput) {
                // create a task for plugin output
            ...
            if (publishApk) {
                ...
                if (extension.publishNonDefault) {
                    ...
                    if (variantOutputData.mainOutputFile.getFilter(OutputFile.DENSITY) != null) {
                        classifier =
                                "${classifier}-${variantOutputData.mainOutputFile.getFilter(OutputFile.DENSITY)}"
                    }
                    if (variantOutputData.mainOutputFile.getFilter(OutputFile.ABI) != null) {
                        classifier =
                                "${classifier}-${variantOutputData.mainOutputFile.getFilter(OutputFile.ABI)}"
                    }
    ...
    public void createProguardTasks(
        ...
        if (testedVariantData != null) {
            ...
            proguardTask.applymapping(
                    "${project.buildDir}/${FD_OUTPUTS}/mapping/${testedVariantData.variantConfiguration.dirName}/mapping.txt")
        }
        ...
        if (variantData instanceof LibraryVariantData) {
            ...
            Closure inJars = {
                Arrays.asList(BasePlugin.getLocalJarFileList(variantData.variantDependency))
            }

            proguardTask.injars(inJars, filter: '!META-INF/MANIFEST.MF')

            // libjar: the library dependencies. In plugin case we take all the compile-scope
            // dependencies
            Closure libJars = {
                Set<File> compiledJars = androidBuilder.getCompileClasspath(variantConfig)
                Object[] localJars = BasePlugin.getLocalJarFileList(variantData.variantDependency)
        ...
        // libraryJars: the runtime jars. Do plugin in doFirst since the boot classpath isn't
    ...
    public void createAnchorTasks(
        ...
        // for all libraries required by the configurations of plugin variant, make plugin task
