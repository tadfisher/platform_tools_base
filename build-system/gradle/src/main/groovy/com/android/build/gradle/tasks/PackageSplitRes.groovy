...
class PackageSplitRes extends IncrementalTask {
    ...
    ImmutableList<VariantOutput> mOutputFiles;
    ...
    @NonNull
    public synchronized  ImmutableList<VariantOutput> getOutputFiles() {
        if (mOutputFiles == null) {
            GsonBuilder gsonBuilder = new GsonBuilder();
            gsonBuilder.registerTypeAdapter(VariantOutput.SplitVariantOutput,
                    new VariantOutput.SplitVariantOutput.JsonDeserializer())
            Gson gson = gsonBuilder.create()

            ImmutableList.Builder<VariantOutput> builder = ImmutableList.builder();

            for (VariantOutput vo : gson.fromJson(
                    new FileReader(getOutputPackagedSplitResListFile()),
                    VariantOutput.SplitVariantOutput[].class)) {
                builder.add(vo);
            }
    ...
    protected void doFullTaskAction() {
        ...
        ImmutableCollection.Builder<VariantOutput> tmpOutputs =
                ImmutableList.builder();

        VariantOutput.SplitVariantOutput[] variantOutputs = gson.fromJson(
                new FileReader(getInputSplitResListFile()), VariantOutput.SplitVariantOutput[].class)

        for (VariantOutput.SplitVariantOutput variantOutput : variantOutputs) {
            ...
            tmpOutputs.add(new VariantOutput.SplitVariantOutput(
                    VariantOutput.OutputType.SPLIT,
                    VariantOutput.SplitType.DENSITY,
