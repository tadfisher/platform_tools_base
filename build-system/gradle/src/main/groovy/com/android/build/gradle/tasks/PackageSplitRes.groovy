...
class PackageSplitRes extends BaseTask {
    ...
    ImmutableList<SplitApkOutput> mOutputFiles;
    ...
    @NonNull
    public synchronized  ImmutableList<SplitApkOutput> getOutputSplitFiles() {
        if (mOutputFiles == null) {
            ...
            if (getOutputPackagedSplitResListFile().exists()) {
                ...
                gsonBuilder.registerTypeAdapter(SplitApkOutput, new SplitApkOutput.JsonDeserializer())
                gsonBuilder.registerTypeAdapter(SplitApkOutput, new SplitApkOutput.JsonDeserializer())
                Gson gson = gsonBuilder.create()
                for (SplitApkOutput vo : gson.fromJson(
                ...
                for (String split : splits) {
                    SplitApkOutput apkOutput = new SplitApkOutput(
    ...
    protected void doFullTaskAction() {
        ...
        GsonBuilder gsonBuilder = new GsonBuilder();
        gsonBuilder.registerTypeAdapter(SplitApkOutput,
                new SplitApkOutput.JsonDeserializer())
        Gson gson = gsonBuilder.create()

        ImmutableCollection.Builder<SplitApkOutput> tmpOutputs =
                ImmutableList.builder();

        SplitApkOutput[] variantOutputs = gson.fromJson(
                new FileReader(getInputSplitResListFile()), SplitApkOutput[].class)

        for (SplitApkOutput variantOutput : variantOutputs) {
            ...
            tmpOutputs.add(new SplitApkOutput(
                    SplitOutput.OutputType.SPLIT,
