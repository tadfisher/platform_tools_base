...
public class LintOptionsImpl implements LintOptions, Serializable {
    ...
    public LintOptionsImpl(
        ...
        if (severityOverrides != null) {
            for (Map.Entry<String,String> entry : severityOverrides.entrySet()) {
                String id = entry.key
                Severity severity = Severity.fromName(entry.value)
                if (severity != null) {
    ...
    public Map<String, String> getSeverityOverrides() {
        ...
        Map<String, String> map = Maps.newHashMapWithExpectedSize(severities.size())
        for (Map.Entry<String,Severity> entry : severities.entrySet()) {
            String id = entry.key
            Severity value = entry.value
