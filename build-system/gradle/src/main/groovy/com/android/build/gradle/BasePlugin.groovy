...
public abstract class BasePlugin {
    ...
    */
    ...
    private void resolveDependencyForConfig(
        ...
        // since we reuse the same instance of LibraryDependencyImpl for identical modules
        // we can simply run through each list and look for libs that are in only one.
        List<LibraryDependencyImpl> copyOfPackagedLibs = Lists.newArrayList(packagedAndroidLibraries)
            ...
            if (!copyOfPackagedLibs.contains(lib)) {
                handleSyncError(
                        ...
                        "Project ${project.name}: provided dependencies can only be jars. " +
                                "${lib.resolvedCoordinates} is not an Android Library.")
             } else {
                copyOfPackagedLibs.remove(lib);
            }
        }
        // at this stage copyOfPackagedLibs should be empty.
