...
public abstract class BasePlugin {
        ...
        for (BaseVariantOutputData vod : variantData.outputs) {
            ...
            if (variantOutputData.mainOutputFile.getFilter(OutputFile.DENSITY) == null
                ...
                if (config.buildType.runProguard) {
                    ...
                    if (config.buildType.shrinkResources && extension.getUseOldManifestMerger()) {
                        displayWarning(logger, project,
                                "Cannot use shrinkResources with the old manifest merger");
                    }
    ...
    public void createPostCompilationTasks(@NonNull ApkVariantData variantData) {
        ...
        if (runProguard) {
            ...
            createProguardTasks(variantData, testedVariantData)
    ...
    public void createPackagingTask(
        ...
        for (ApkVariantOutputData vod : variantData.outputs) {
            ...
            if (config.buildType.runProguard && config.buildType.shrinkResources &&
                    !extension.getUseOldManifestMerger()) {
