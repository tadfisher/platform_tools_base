...
public abstract class BasePlugin {
    ...
    public void createProcessResTask(
        ...
        for (BaseVariantOutputData vod : variantData.outputs) {
            ...
            processResources.conventionMapping.versionCode = {
                String.valueOf(variantOutputData.getVersionCode())
            }
    ...
    private DeviceProviderInstrumentTestTask createDeviceProviderInstrumentTestTask(
        ...
        testTask.conventionMapping.splitApks = {
            ArrayList<File> splits = new ArrayList<>();
            for (APKOutput variantOutput : testedVariantOutputData.getOutputFiles()) {
                if (variantOutput.getType() == APKOutput.OutputType.SPLIT) {
                    splits.add(variantOutput.getOutputFile())
                }
            }
            return splits.toArray(new File[splits.size()]);
        }
