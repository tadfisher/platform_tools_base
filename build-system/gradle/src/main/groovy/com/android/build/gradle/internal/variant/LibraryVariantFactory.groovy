...
import static com.android.build.gradle.BasePlugin.FD_O
...
public class LibraryVariantFactory implements VariantFactory {
    ...
    public void createTasks(@NonNull BaseVariantData variantData, @Nullable Task assembleTask) {
        ...
        MergeResources packageRes = basePlugin.basicCreateMergeResourcesTask(variantData,
                "package",
                "$project.buildDir/${FD_I}/$DIR_BUNDLES/${dirName}/res",
        ...
        basePlugin.createMergeAssetsTask(variantData,
                "$project.buildDir/${FD_I}/$DIR_BUNDLES/${dirName}/assets",
                false /*includeDependencies*/)
        ...
        basePlugin.createProcessResTask(variantData,
                "$project.buildDir/${FD_I}/$DIR_BUNDLES/${dirName}",
        ...
        basePlugin.createAidlTask(variantData, basePlugin.project.file(
                "$basePlugin.project.buildDir/${FD_I}/$DIR_BUNDLES/${dirName}/$SdkConstants.FD_AIDL"))
        ...
        packageJniLibs.into(project.file(
                "$project.buildDir/${FD_I}/$DIR_BUNDLES/${dirName}/jni"))
        ...
        packageRenderscript.into(project.file(
                "$project.buildDir/${FD_I}/$DIR_BUNDLES/${dirName}/$SdkConstants.FD_RENDERSCRIPT"))
        ...
        mergeProGuardFileTask.conventionMapping.outputFile = {
            project.file(
                    "$project.buildDir/${FD_I}/$DIR_BUNDLES/${dirName}/$LibraryBundle.FN_PROGUARD_TXT")
        }
        ...
        lintCopy.from("$project.buildDir/${FD_I}/lint/lint.jar")
        lintCopy.into("$project.buildDir/${FD_I}/$DIR_BUNDLES/$dirName")
        ...
        if (buildType.runProguard) {
            ...
            if (instrumented) {
                ...
                jacocoTask.conventionMapping.outputDir = { project.file("${project.buildDir}/${FD_I}/coverage-instrumented-classes/${variantConfig.dirName}") }
            ...
            packageLocalJar.into(project.file(
                    "$project.buildDir/${FD_I}/$DIR_BUNDLES/${dirName}/$LIBS_FOLDER"))
        ...
        bundle.destinationDir = project.file("$project.buildDir/${FD_O}/aar")
        bundle.extension = BuilderConstants.EXT_LIB_ARCHIVE
        bundle.from(project.file("$project.buildDir/${FD_I}/$DIR_BUNDLES/${dirName}"))
        ...
        variantConfig.output = new LibraryBundle(
                bundle.archivePath,
                project.file("$project.buildDir/${FD_I}/$DIR_BUNDLES/${dirName}"),
                variantData.getName()) {
    ...
    public Task createExtractAnnotations(
        ...
        task.destinationDir = project.file("$project.buildDir/${FD_I}/$DIR_BUNDLES/${dirName}")
        task.output = new File(task.destinationDir, FN_ANNOTATIONS_ZIP)
        task.classDir = project.file("$project.buildDir/${FD_I}/classes/${variantData.variantConfiguration.dirName}")
