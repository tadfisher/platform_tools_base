...
public class TVDetector extends ResourceXmlDetector implements Detector.XmlScanner {
    ...
    private static final String HARDWARE_FEATURE_NFC = "android.hardware.nfc";
    ...
    private static final String HARDWARE_FEATURE_SENSORS = "android.hardware.sensors";
    ...
    private static final String SOFTWARE_FEATURE_LEANBACK = "android.software.leanback";
    ...
    private static final String PERMISSION_WRITE_SMS = "android.permission.WRITE_SMS";
    ...
    private static final Map<String, String> PERMISSIONS_TO_IMPLIED_UNSUPPORTED_HARDWARE =
            ImmutableMap.<String, String>builder().put(PERMISSION_BLUETOOTH, HARDWARE_FEATURE_BLUETOOTH)
    ...
    public void afterCheckFile(@NonNull Context context) {
        if (mIsTVApp && context.getPhase() == 2) {
    ...
    private void checkTVIssues(@NonNull XmlContext context, @NonNull Element element) {
        ...
        if (NODE_USES_FEATURE.equals(elementName)) {
            ...
            if (nameNode != null) {
                ...
                if (PERMISSIONS_TO_IMPLIED_UNSUPPORTED_HARDWARE.containsKey(name)
                        && context.isEnabled(PERMISSION_IMPLIES_UNSUPPORTED_HARDWARE)) {
                    Attr required =
                            element.getAttributeNodeNS(ANDROID_URI, ATTRIBUTE_REQUIRED);
                    if (required == null || Boolean.parseBoolean(required.getValue())) {
                        Location location =
                                context.getLocation((required == null) ? element : required);
                        String impliedHardware =
                                PERMISSIONS_TO_IMPLIED_UNSUPPORTED_HARDWARE.get(name);
                        context.report(
                                PERMISSION_IMPLIES_UNSUPPORTED_HARDWARE,
                                location,
                                "Expecting implied hardware feature to have `" +
                                        PREFIX_ANDROID +
                                        ATTRIBUTE_REQUIRED +
                                        "=\"" +
                                        false +
