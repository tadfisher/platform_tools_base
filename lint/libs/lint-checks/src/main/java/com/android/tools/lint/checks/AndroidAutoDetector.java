...
public class AndroidAutoDetector extends ResourceXmlDetector
    ...
    public Collection<String> getApplicableElements() {
        return Arrays.asList(
                ...
                NODE_APPLICATION    // Used for checking if an issue is suppressed.
        );
    }
    ...
    public void visitElement(@NonNull XmlContext context, @NonNull Element element) {
        ...
        if (context.getPhase() == 1) {
            ...
            // and if necessary request a second phase for processing the service elements.
                ...
                if (NODE_APPLICATION.equals(tagName)) {
                    ...
                    if (context.getMainProject() == context.getProject()
                        ...
                        if (context.getDriver().isSuppressed(
                                context, MISSING_INTENT_FILTER_FOR_MEDIA_SEARCH, element)) {
                            mMediaSearchIntentFilterFound = true;
                        }
                        if (context.getDriver().isSuppressed(context,
                                MISSING_MEDIA_BROWSER_SERVICE_ACTION_ISSUE, element)) {
