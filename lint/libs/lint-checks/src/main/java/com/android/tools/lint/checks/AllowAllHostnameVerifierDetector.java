...
public class AllowAllHostnameVerifierDetector extends Detector
            ...
            if ("verify".equals(method.name)) {
                ...
                if (containsIconst1 && emptyMethod) {
                    ...
                    context.report(ISSUE, location, method.name + " always returns true, which " +
                            "could cause insecure network traffic due to trusting arbitrary " +
                            "hostnames in certificates presented by peers");
                }
            }
        }
    }
    ...
    public List<String> getApplicableMethodNames() {
        return Arrays.asList("setHostnameVerifier");
    }
    ...
    private static class IdentifierVisitor extends ForwardingAstVisitor {
        ...
        private Set<ConstructorInvocation> mVisitedConstructorInvocations;

        public IdentifierVisitor(JavaContext context) {
            super();
            mContext = context;
            mVisitedConstructorInvocations = new HashSet<ConstructorInvocation>();
        }

        @Override
        public boolean visitConstructorInvocation(ConstructorInvocation node) {
            if (mVisitedConstructorInvocations.contains(node)) {
                return false;
            }
            mVisitedConstructorInvocations.add(node);
            if ("AllowAllHostnameVerifier".equals(node.astTypeReference().getTypeName())) {
                ...
                mContext.report(ISSUE, node, location,
                        ...
                        "traffic due to trusting arbitrary hostnames in certificates " +
                        "presented by peers.");
