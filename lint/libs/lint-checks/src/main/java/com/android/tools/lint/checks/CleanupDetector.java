        ...
        public boolean visitMethodInvocation(MethodInvocation call) {
            ...
            if (!mEscapes) {
                for (Expression expression : call.astArguments()) {
                    if (expression instanceof VariableReference && mVariableName
                            .equals((((VariableReference) expression).astIdentifier()
                                    .astValue()))) {
                        ResolvedNode resolved = mContext.resolve(expression);
                        if (resolved != null && resolved.equals(mVariable)) {
                            mEscapes = true;

                            // Special case: MotionEvent.obtain(MotionEvent): passing in an event
                            // here does not recycle the event, and we also know it doesn't escape
                            if (OBTAIN.equals(call.astName().astValue())) {
                                ...
                                if (r instanceof ResolvedMethod) {
                                    ...
                                    if (method.getContainingClass().matches(MOTION_EVENT_CLS)) {
                                        mEscapes = false;
                                    }
                                }
                            }
