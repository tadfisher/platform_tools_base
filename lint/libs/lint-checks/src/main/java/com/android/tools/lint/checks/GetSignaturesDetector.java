...
public class GetSignaturesDetector extends Detector implements Detector.JavaScanner  {
    ...
    public void visitMethod(@NonNull JavaContext context, @Nullable AstVisitor visitor,
        ...
        if (!(resolved instanceof JavaParser.ResolvedMethod) ||
                !((JavaParser.ResolvedMethod) resolved).getContainingClass()
        ...
        if (argumentList != null && argumentList.size() == 2
                && isStringParameter(argumentList.first(), context)) {
    ...
    private static int calculateValue(JavaContext context, Expression expression) {
        ...
        if (resolvedNode instanceof JavaParser.ResolvedField) {
            Object value = ((JavaParser.ResolvedField) resolvedNode).getValue();
        ...
        if (expression instanceof BinaryExpression) {
            ...
            String operator = binaryExpression.astOperator().getSymbol();
