...
public class SQLiteDetector extends Detector implements Detector.JavaScanner {
    ...
    public static final Issue ISSUE = Issue.create(
            ...
            "string.\n" +
            "\n" +
            "This can lead to some subtle bugs. For example, a string like `1e2` appears to be " +
            "a float, but if it is out of range for floating point numbers, it will appear as " +
            "`Inf`! Similarly, strings that look like integers will lose leading zeroes.\n" +
            "\n" +
        ...
        if (method.getArgumentCount() > 0
            ...
            if (sql != null && (sql.startsWith("CREATE TABLE") || sql.startsWith("ALTER TABLE"))
                ...
                String message = "Using column type STRING; did you mean to use TEXT? "
                        ...
                        + "strings that looks like integers can drop leading zeroes. See issue "
