...
public class RelativeOverlapDetector extends LayoutDetector {
	public static final Issue ISSUE = Issue.create("RelativeOverlap",
	...
	private static class LayoutNode {
		...
		public String getNodeId() {
			...
			if (nodeid.isEmpty()) {
				return String.format("%1$s-%2$d", mNode.getNodeName(), mIndex);
			} else {
		...
		public boolean fixedWidth() {
			...
			if (width.equals(VALUE_WRAP_CONTENT)) {
				...
				// If node contains text attribute, consider if fixed-width if
				// text is
				// hard-coded, otherwise it is not fixed-width.
				String text = mNode.getAttributeNS(ANDROID_URI, ATTR_TEXT);
				if (!text.isEmpty()) {
					return text.charAt(0) != '@';
				}

				String nodeName = mNode.getNodeName();
		...
		public boolean skip() {
			...
			if (mNode.getNodeName().equals(VIEW_INCLUDE)
					|| mNode.getNodeName().equals(VIEW)) {
				return true;
			}
			return false;
		}
		...
		private boolean hasAttr(@NonNull String key) {
			return !mNode.getAttributeNS(ANDROID_URI, key).isEmpty();
		}
		...
		for (LayoutNode right : nodes.values()) {
			...
			for (LayoutNode left : nodes.values()) {
				...
				if (canGrowLeft.size() > 0 || canGrowRight.size() > 0) {
					...
					context.report(ISSUE, nodeToBlame.getNode(), context
							.getLocation(nodeToBlame.getNode()), String.format(
							"%1$s can overlap %2$s if %3$s %4$s due to localized text expansion", nodeToBlame.getNodeId(), otherNode.getNodeId(), Joiner.on(", ").join(canGrowRight), canGrowRight.size() > 1 ? "grow"
									: "grows"), null);
				}
			}
		}
