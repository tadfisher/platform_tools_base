...
public class ApiDetector extends ResourceXmlDetector
                ...
                if (attributeApiLevel > minSdk && attributeApiLevel > context.getFolderVersion()
                    ...
                    if (RtlDetector.isRtlAttributeName(name)) {
                        ...
                        if (name.equals(ATTR_PADDING_START) &&
                                !notTextView(attribute.getOwnerElement())) {
    ...
    /**
     * Returns true if the view tag is known <b>not</b> to represent a
     * text view. (This is expressed as a negative assertion rather than
     * a positive assertion because it may not be certain whether a view
     * is a text view, it only determines whether we know that a text view
     * is *not* a text view.
     */
    private static boolean notTextView(@NonNull Element element) {
