...
public class TypoDetector extends ResourceXmlDetector {
    ...
    private void check(XmlContext context, Element element, Node node, byte[] utf8Text,
        ...
        while (index < byteEnd) {
            ...
            if (lastWordBegin != -1 && charEnd - charStart == lastWordEnd - lastWordBegin &&
                    charEnd - charStart > 1) {
                // See whether we have a repeated word
                boolean different = false;
                for (int i = lastWordBegin, j = charStart; i < lastWordEnd; i++, j++) {
                    if (text.charAt(i) != text.charAt(j)) {
                        different = true;
                    }
                }
                if (!different && onlySpace(text, lastWordEnd, charStart)
                        && isTranslatable(element)) {
                    reportRepeatedWord(context, node, text, lastWordBegin, charStart, charEnd);
                }
            }
    ...
    private void reportRepeatedWord(XmlContext context, Node node, String text, int lastWordBegin,
            int begin, int end) {
