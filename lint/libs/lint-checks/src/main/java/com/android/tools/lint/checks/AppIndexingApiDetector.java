...
public class AppIndexingApiDetector extends Detector implements Detector.XmlScanner {
  ...
  public void afterCheckProject(@NonNull Context context) {
    if (mActivityLocations.size() > 0 && !mHasActionView) {
      for (Location location : mActivityLocations) {
        context.report(ISSUE, location, "Need an <action> tag that specifies the ACTION_VIEW intent action.");
      }
    }
  }
          ...
          if (child.getNodeName().equals(TAG_INTENT_FILTER)) {
            if (child.hasChildNodes()) {
              ...
              for (int j = 0; j < grandChildren.getLength(); j++) {
                ...
                if (grandChild.getNodeType() == Node.ELEMENT_NODE && grandChild.getNodeName().equals(TAG_ACTION)) {
              ...
              if (isActionView) {
                ...
                if (!isBrowsable) {
                  context.report(ISSUE, child, context.getLocation(child), "Activity supporting ACTION_VIEW is not set as browsable");
                }
              }
            }
          }
  ...
  private void checkApplication(XmlContext context, Element element) {
    boolean hasGMS = false;
    if (element.hasChildNodes()) {
      NodeList children = element.getChildNodes();
      for (int i = 0; i < children.getLength(); i++) {
        Node child = children.item(i);
        if (child.getNodeType() == Node.ELEMENT_NODE && child.getNodeName().equals(TAG_META_DATA)) {
          Element e = (Element)child;
          if (e.hasAttributeNS(ANDROID_URI, ATTR_NAME)) {
            Attr attr = e.getAttributeNodeNS(ANDROID_URI, ATTR_NAME);
            if (attr.getValue().startsWith("com.google.android.gms.")) {
              hasGMS = true;
            }
          }
        }
    ...
    if (!hasGMS) {
      context.report(ISSUE, element, context.getLocation(element), "Google Play Services not included");
    }
  }
  ...
  private void checkData(XmlContext context, Element element) {
    if (!element.hasAttributeNS(ANDROID_URI, ATTR_SCHEME)) {
      context
        .report(ISSUE, element, context.getLocation(element), "At least one android:scheme attribute should be set for the intent filter");
    }

    if (!element.hasAttributeNS(ANDROID_URI, ATTR_HOST)) {
      context.report(ISSUE, element, context.getLocation(element), "android:host attribute must be set for the intent filter");
    }
    ...
    if (element.hasAttributeNS(ANDROID_URI, ATTR_PATH_PREFIX)) {
      ...
      if (!pathPrefix.getValue().startsWith("/")) {
        context.report(ISSUE, pathPrefix, context.getLocation(pathPrefix), "android:pathPrefix attribute should start with /");
      }
    }
  }
}
