...
public class AppIndexingApiDetector extends Detector
    ...
    public static final Issue ISSUE_DEEP_LINK = Issue.create("DeepLinkError", //$NON-NLS-1$
            "Wrong Usage of Deep Link",
            "Ensure the deep link is correctly defined.",
    ...
    public static final Issue ISSUE_APP_INDEXING_ERROR =
            Issue.create(
                    ...
                    "Ensure the app can correctly handle deep links and integrate "
                            + "with App Indexing for Google search.",
                    Category.USABILITY,
                    5,
                    Severity.ERROR,
                    IMPLEMENTATION_JAVA)
    ...
    public void visitElement(@NonNull XmlContext context, @NonNull Element intent) {
        ...
        if (intent.getNodeName().equals(NODE_APPLICATION)) {
            if (!hasGMS(intent)) {
                context.report(ISSUE_APP_INDEXING_ERROR, intent, context.getLocation(intent),
        ...
        if ((hasPath || hasHost || hasPort) && !hasScheme) {
            context.report(ISSUE_DEEP_LINK, firstData, context.getLocation(firstData),
        ...
        if (actionView && browsable) {
            if (firstData == null) {
                ...
                context.report(ISSUE_DEEP_LINK, intent, context.getLocation(intent),
                        "Missing data node?");
        ...
        if (actionView && isHttp && !browsable) {
            context.report(ISSUE_APP_INDEXING_WARNING, intent, context.getLocation(intent),
        ...
        for (String name : PATH_ATTR_LIST) {
            if (data.hasAttributeNS(ANDROID_URI, name)) {
                ...
                if (!path.startsWith("/") && !path.startsWith(SdkConstants.PREFIX_RESOURCE_REF)) {
                    context.report(ISSUE_DEEP_LINK, attr, context.getLocation(attr),
            ...
            if (classDeclare != null) {
                ...
                for (MethodInvocation startNode : startMethod) {
                    ...
                    if (!hasOperand(startClient, connectMethod)) {
                        ...
                        javaContext.report(ISSUE_APP_INDEXING_ERROR, startClient,
                ...
                for (MethodInvocation endNode : endMethod) {
                    ...
                    if (!hasOperand(endClient, disconnectMethod)) {
                        String message = String.format(
                                "GoogleApiClient `%1$s` should disconnect afterwards",
                                endClient.toString());
                        javaContext.report(ISSUE_APP_INDEXING_ERROR, endClient,
        ...
        if (body != null) {
            for (TypeMember member : body.astMembers()) {
                if (member instanceof VariableDeclaration) {
                    ...
                    if (typeReference.toString().endsWith(type)) {
                        return true;
                    }
                }
            }
