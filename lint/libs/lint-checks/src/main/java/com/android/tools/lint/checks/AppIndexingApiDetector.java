...
public class AppIndexingApiDetector extends Detector
    ...
    private static final Implementation IMPLEMENTATION_JAVA =
            new Implementation(AppIndexingApiDetector.class,
                    EnumSet.of(Scope.JAVA_FILE, Scope.MANIFEST), Scope.JAVA_FILE_SCOPE);

    public static final Issue ISSUE_ERROR =
            Issue.create(
                    "AppIndexingError", //$NON-NLS-1$
    ...
    public static final Issue ISSUE_JAVA_ERROR =
            Issue.create(
                    "AppIndexingJavaError", //$NON-NLS-1$
    ...
    public static final Issue ISSUE_JAVA_WARNING =
            Issue.create(
                    "AppIndexingJavaWARNING", //$NON-NLS-1$
    ...
    private static final String[] PATH_ATTR_LIST =
            new String[] {ATTR_PATH_PREFIX, ATTR_PATH, ATTR_PATH_PATTERN};
    ...
    public void visitElement(@NonNull XmlContext context, @NonNull Element intent) {
        ...
        if (intent.getNodeName().equals(NODE_APPLICATION)) {
            if (!hasGMS(intent)) {
                context.report(
                        ISSUE_WARNING, intent, context.getLocation(intent), "GMS should be added to the app");
            }
            return;
        }
        ...
        if ((hasPath || hasHost || hasPort) && !hasScheme) {
            context.report(
                    ISSUE_ERROR, firstData, context.getLocation(firstData), "android:scheme missing");
        }

        if ((hasPath || hasPort) && !hasHost) {
            context.report(
                    ISSUE_ERROR, firstData, context.getLocation(firstData), "android:host missing");
        }
    ...
    private static void checkSingleData(XmlContext context, Element data) {
            ...
            if (data.hasAttributeNS(ANDROID_URI, name)) {
                ...
                if (!path.startsWith("/") && !path.startsWith(SdkConstants.PREFIX_RESOURCE_REF)) {
                    context.report(
                            ...
                            "android:" + name + " attribute should start with '/', but it is : " + path);
                }
            }
        }
        ...
        if (data.hasAttributeNS(ANDROID_URI, ATTRIBUTE_PORT)) {
            ...
            try {
                ...
                context.report(
                        ISSUE_ERROR, attr, context.getLocation(attr), "android:port is not a legal number");
            }
        }
        ...
        for (int i = 0; i < attrs.getLength(); i++) {
            ...
            if (item.getNodeType() == Node.ATTRIBUTE_NODE) {
                ...
                if (attr.getValue().isEmpty()) {
                    context.report(
                            ISSUE_ERROR, attr, context.getLocation(attr), attr.getName() + " cannot be empty");
                }
            }
        }
    }
    ...
    public List<String> getApplicableMethodNames() {
        return Arrays.asList(APP_INDEX_START, APP_INDEX_END, CLIENT_CONNECT, CLIENT_DISCONNECT);
    }

    @Override
    public void visitMethod(
            @NonNull JavaContext context, @Nullable AstVisitor visitor, @NonNull MethodInvocation node) {
            ...
            if (classDeclare != null) {
                ...
                for (MethodInvocation startNode : startMethod) {
                    ...
                    if (!hasOperand(startClient, connectMethod)) {
                        String message =
                                String.format("GoogleApiClient `%1$s` has not connected", startClient.toString());
                        javaContext.report(
                                ISSUE_JAVA_ERROR, startClient, javaContext.getLocation(startClient), message);
                    }
                ...
                for (MethodInvocation endNode : endMethod) {
                    ...
                    if (!hasOperand(endClient, disconnectMethod)) {
                        String message =
                                String.format("GoogleApiClient `%1$s` has not connected", endClient.toString());
                        javaContext.report(
                                ISSUE_JAVA_ERROR, endClient, javaContext.getLocation(endClient), message);
                    }
