...
public class AppIndexingApiDetector extends Detector implements Detector.XmlScanner {
    ...
    public void visitElement(@NonNull XmlContext context, @NonNull Element activity) {
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (child.getNodeType() == Node.ELEMENT_NODE &&
                ...
                for (int j = 0; j < grandChildren.getLength(); j++) {
                    ...
                    if (grandChild.getNodeType() == Node.ELEMENT_NODE) {
                        ...
                        if (e.getNodeName().equals(NODE_DATA)) {
                            ...
                            // If this activity is an action view, is browsable, but doesn't have a
                            // URL, it may be a mistake and report warning.
                            if (actionView && browsable &&
                                    !e.hasAttributeNS(ANDROID_URI, ATTR_SCHEME)) {
                                context.report(ISSUE_WARNING, e, context.getLocation(e),
                                        "android:scheme should be set for the intent filter");
                ...
                // If this activity is an action view, has a http URL but missing browsable, it may
                // be a mistake and report warning.
        ...
        for (int i = 0; i < children.getLength(); i++) {
            ...
            if (child.getNodeType() == Node.ELEMENT_NODE &&
                ...
                for (int j = 0; j < grandChildren.getLength(); j++) {
                    Node grandChild = grandChildren.item(j);
                    if (grandChild.getNodeType() == Node.ELEMENT_NODE &&
                            grandChild.getNodeName().equals(NODE_ACTION)) {
    ...
    private static void checkData(XmlContext context, Element data) {
        ...
        for (String name : PATH_ATTR_TO_CHECK) {
            if (data.hasAttributeNS(ANDROID_URI, name)) {
                ...
                if (hasIllegalChar(attr.getValue())) {
                    context.report(ISSUE_ERROR, attr, context.getLocation(attr),
                            "android:" + name + " contains illegal character");
                }
            }
        }
        ...
        if (data.hasAttributeNS(ANDROID_URI, ATTR_SCHEME)) {
            hasScheme = true;
            Attr attr = data.getAttributeNodeNS(ANDROID_URI, ATTR_SCHEME);
            if (hasIllegalChar(attr.getValue())) {
                context.report(ISSUE_ERROR, attr, context.getLocation(attr),
                        "android:scheme contains illegal character");
            }
        }

        if (data.hasAttributeNS(ANDROID_URI, ATTR_HOST)) {
            hasHost = true;
            Attr attr = data.getAttributeNodeNS(ANDROID_URI, ATTR_HOST);
            if (hasIllegalChar(attr.getValue())) {
                context.report(ISSUE_ERROR, attr, context.getLocation(attr),
                        "android:host contains illegal character");
            }
        }
        ...
        for (int i = 0; i < attrs.getLength(); i++) {
            ...
            if (item.getNodeType() == Node.ATTRIBUTE_NODE) {
                ...
                if (attr.getValue().isEmpty()) {
                    context.report(ISSUE_ERROR, attr, context.getLocation(attr),
                            attr.getName() + " should be specified");
                }
            }
        }
    }
    ...
    // Legal character set of a URL.
    private static final Set<Character> CHARACTER_SET;

    static {
        CHARACTER_SET = Sets.newHashSet();
        for (Character ch : ("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
