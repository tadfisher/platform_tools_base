...
public class AllowAllHostnameVerifierDetectorTest extends AbstractCheckTest {
    ...
    public void testBroken() throws Exception {
        assertEquals(
            "src/test/pkg/InsecureHostnameVerifier.java:21: Warning: verify always returns true, which could cause insecure network traffic due to trusting arbitrary hostnames in certificates presented by peers [AllowAllHostnameVerifier]\n" +
            "        public boolean verify(String hostname, SSLSession session) {\n" +
            "                       ~~~~~~\n" +
            "src/test/pkg/InsecureHostnameVerifier.java:36: Warning: Using the AllowAllHostnameVerifier HostnameVerifier is unsafe because it always returns true, which could cause insecure network traffic due to trusting arbitrary hostnames in certificates presented by peers. [AllowAllHostnameVerifier]\n" +
            "            connection.setHostnameVerifier(new AllowAllHostnameVerifier());\n" +
            "                                           ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n" +
            "src/test/pkg/InsecureHostnameVerifier.java:37: Warning: Using the ALLOW_ALL_HOSTNAME_VERIFIER HostnameVerifier is unsafe because it always returns true, which could cause insecure network traffic due to trusting arbitrary hostnames in certificates presented by peers. [AllowAllHostnameVerifier]\n" +
