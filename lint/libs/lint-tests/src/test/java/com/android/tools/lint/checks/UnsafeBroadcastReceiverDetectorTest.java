...
public class UnsafeBroadcastReceiverDetectorTest extends AbstractCheckTest {
    ...
    public void testBroken() throws Exception {
        assertEquals(
            "src/test/pkg/TestReceiver.java:10: Warning: Ensure that the receiver calls " +
            "getAction to check that the received intent's action string matches the expected " +
            "value. [UnsafeBroadcastReceiver]\n" +
    ...
    public void testBroken2() throws Exception {
        assertEquals(
            "AndroidManifest.xml:12: Warning: BroadcastReceivers that declare an intent-filter " +
            "for SMS_DELIVER or SMS_RECEIVED must ensure that the caller has the BROADCAST_SMS " +
            "permission, otherwise it is possible for malicious actors to spoof intents. " +
