 ...
 */
public class SyntheticHeap {

    private final Heap mHeap;

    private final ClassInstance[] mNodes;

    public SyntheticHeap(int numNodes) {
    ...
    public SyntheticHeap addEdge(int nodeA, int nodeB) {
        Value link = new Value(mNodes[nodeA]);
        link.setValue(mNodes[nodeB]);
        mNodes[nodeA].addField(new Field(Type.OBJECT, "f" + nodeB), link);
        return this;
    }

    public SyntheticHeap addRoot(int node) {
