    ...
    public void testMethodProfileData() throws IOException {
        ...
        for (MethodInfo method : traceData.getMethods().values()) {
            ...
            if (!isTop) {
                ...
                assertEquals(profile.getInvocationCount(thread), sumCallerInvocationCounts(profile,
                        thread));
            }
        }
    }

    private long sumCallerInvocationCounts(MethodProfileData profile, ThreadInfo thread) {
        long sum = 0;
