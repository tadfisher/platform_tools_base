...
public class VmTraceParserTest extends TestCase {
    ...
    private class MethodNameFormatter implements Call.MethodFormatter {
        private final Map<Long, MethodInfo> mMethodInfo;

        public MethodNameFormatter(Map<Long, MethodInfo> methodInfo) {
            mMethodInfo = methodInfo;
        }

        @Override
        public String format(long methodId) {
            MethodInfo info = mMethodInfo.get(methodId);
            if (info == null) {
                return Long.toString(methodId);
            }

            return String.format(Locale.US, "%s.%s: %s", info.className, info.methodName, info.signature);
        }
    }
    ...
    private void testTrace(String traceName, String threadName, String expectedCallSequence) throws IOException {
        ...
        int threadId = findThreadByName(threadName, traceData.getThreads());
        ...
        String actual = formatCallStacks(calls, new MethodNameFormatter(traceData.getMethods()));
    ...
    private int findThreadByName(@NotNull String threadName, @NotNull SparseArray<String> threads) {
    ...
    private String formatCallStacks(List<Call> calls, Call.MethodFormatter formatter) {
        if (calls == null) return "<none>";
        List<String> callStacks = new ArrayList<String>(calls.size());

        for (Call c : calls) {
            callStacks.add(c.toString(formatter));
        }
