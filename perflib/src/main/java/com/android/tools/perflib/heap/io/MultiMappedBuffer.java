...
class MultiMappedBuffer extends HprofBuffer {

    // Eliminate wrapped, multi-byte reads across chunks in most cases.
    static int PADDING = 1024;

    ByteBuffer[] mByteBuffers;

    long mCurrentPosition;

    long mLength;

    MultiMappedBuffer(File f) throws IOException {
        mLength = f.length();
        int shards = (int) (mLength / BUFFER_SIZE) + 1;
        ...
        try {
            ...
            for (int i = 0; i < shards; i++) {
                long size = Math.min(mLength - offset, BUFFER_SIZE + PADDING);
    ...
    public void read(byte[] b) {
        ...
        if (b.length <= mByteBuffers[index].remaining()) {
            ...
            // Wrapped read
            int split = BUFFER_SIZE - mByteBuffers[index].position();
    ...
    private int getIndex() {
        return (int) (mCurrentPosition / BUFFER_SIZE);
    }
