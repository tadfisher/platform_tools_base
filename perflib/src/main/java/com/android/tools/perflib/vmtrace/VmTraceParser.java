    ...
    private void computePerThreadStats(@NonNull ThreadInfo thread, VmTraceData data) {
        ...
        while (it.hasNext()) {
            ...
            for (ClockType type : ClockType.values()) {
                info.addExclusiveTime(c.getExclusiveTime(type, TimeUnit.NANOSECONDS), thread.getName(), type);
            }
            ...
            if (!c.isRecursive()) {
                ...
                for (ClockType type : ClockType.values()) {
                    info.addInclusiveTime(c.getInclusiveTime(type, TimeUnit.NANOSECONDS), thread.getName(), type);
                }
            }
        }
