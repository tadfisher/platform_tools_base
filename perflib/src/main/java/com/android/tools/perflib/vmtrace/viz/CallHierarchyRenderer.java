...
public class CallHierarchyRenderer {
    ...
    private Rectangle2D transformRect(AffineTransform viewPortTransform, Rectangle2D rect) {
        ...
        mTmpPoint2.setLocation(rect.getX() + rect.getWidth(), rect.getY() + rect.getHeight());

        viewPortTransform.transform(mTmpPoint1, mTmpPoint1);
        viewPortTransform.transform(mTmpPoint2, mTmpPoint2);

        rect.setRect(mTmpPoint1.getX(),
                mTmpPoint1.getY(),
                mTmpPoint2.getX() - mTmpPoint1.getX(),
                mTmpPoint2.getY() - mTmpPoint1.getY());
        return rect;
    }
    ...
    public String getToolTipFor(int x, int y) {
        ...
        while (it.hasNext()) {
            ...
            if (mLayout.contains(x, y)) {
                System.out.println("depth = " + c.getDepth() + ", bound = " + mLayout);
                results.add(c);
            }
        }
        ...
        if (results.isEmpty()) {
            ...
            System.out.println("# of matches: " + results.size());
            for (Call c : results) {
                System.out.printf("start, end: %d, %d\n", c.getEntryTime(mRenderClock, mLayoutTimeUnits),
                        c.getExitTime(mRenderClock, mLayoutTimeUnits));
            }
            return formatToolTip(results.get(0));
        }
    }
