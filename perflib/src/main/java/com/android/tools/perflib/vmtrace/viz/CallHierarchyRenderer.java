...
public class CallHierarchyRenderer {
    ...
    /** TimeUnit to use for layout operations. */
    private static final TimeUnit sLayoutTimeUnits = TimeUnit.NANOSECONDS;
    ...
    private void fillLayoutBounds(Call c, Rectangle2D layoutBounds) {
        double x = c.getEntryTime(mRenderClock, sLayoutTimeUnits)
                - mTopCall.getEntryTime(mRenderClock, sLayoutTimeUnits)
                + PADDING;
        double y = c.getDepth() * PER_LEVEL_HEIGHT_PX + mYOffset + PADDING;
        double width  = c.getInclusiveTime(mRenderClock, sLayoutTimeUnits) - 2 * PADDING;
