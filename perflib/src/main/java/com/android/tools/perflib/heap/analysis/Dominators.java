...
public class Dominators {
    ...
    private final HashMap<Instance, Instance> idom = Maps.newHashMap();

    public Dominators(Heap heap) {
        mHeap = heap;
    }
        ...
        while (changed) {
            ...
            for (Instance node : topSort.keySet()) {
                if (!roots.contains(node)) { // Root nodes are skipped
                    ...
                    if (idom.get(node) != dominator) {
                        idom.put(node, dominator);
                        changed = true;
                    }
                }
            }
