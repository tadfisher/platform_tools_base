...
public class AlternateDominators {
    ...
    private static class DominatorVisitor extends BreadthFirstVisitor {
        ...
        private void processReference(Instance src, Instance dst, String field) {
            ...
            Instance dominator = dst.getImmediateDominator();
            ...
            if (dstpath == null) {
                ...
                ArrayList<PathElem> retpath = getPath(dominator);
                if (retpath == null) {
                    throw new RuntimeException(
                            String.format("retpath is null for @%x from @%x with path %s",
                                dominator.getId(), dst.getId(), dstpath.toString()));
                }
                for (int i = 1; i < retpath.size() && i < srcpath.size(); i++) {
                    if (retpath.get(i) != srcpath.get(i)) {
                        dominator = retpath.get(i-1).object;
                        break;
                    }
                }
