...
public class LocalAddonPkgInfo extends LocalPlatformPkgInfo {
    ...
    protected IAndroidTarget createAndroidTarget() {
        ...
        if (error != null) {
            appendLoadError("Ignoring add-on '%1$s': %2$s", fileOp.getName(getLocalDir()), error);
            return null;
        }
        ...
        try {
            ...
            AddOnTarget target = new AddOnTarget(
                    fileOp.getAbsolutePath(getLocalDir()),
                    name,
                    vendor,
                    revisionValue,
                    description,
            ...
            appendLoadError("Ignoring add-on '%1$s': error %2$s.",
                    fileOp.getName(getLocalDir()), e.toString());
        }
