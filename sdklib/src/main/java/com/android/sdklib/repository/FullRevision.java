...
public class FullRevision implements Comparable<FullRevision> {
    ...
    public FullRevision(int major, int minor, int micro, int preview) {
      this(major, minor, micro, preview, DEFAULT_SEPARATOR, PreviewType.RC);
    }

    public FullRevision(int major, int minor, int micro, int preview,
            @NonNull String previewSeparator, @NonNull PreviewType previewType) {
        mMajor = major;
        mMinor = minor;
        mMicro = micro;
        mPreview = preview;
            ...
            if (m != null && m.matches()) {
                ...
                if (error == null) {
                    if (keepPrecision) {
                        ...
                        return new FullRevision(major, minor, micro, preview, previewSeparator, previewType);
                    }
                }
            }
        } catch (Throwable t) {
