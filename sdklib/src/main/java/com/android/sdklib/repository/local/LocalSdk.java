...
public class LocalSdk {
    ...
    public void clearLocalPkg(int filters) {
        ...
        for (int filter = minf; filters != 0 && filter <= PkgType.PKG_ALL; filter <<= 1) {
            if ((filters & filter) == 0) {
                continue;
            }
            filters ^= filter;
    ...
    public LocalPkgInfo getPkgInfo(int filter) {
        ...
        switch(filter) {
        case PkgType.PKG_TOOLS:
        case PkgType.PKG_PLATFORM_TOOLS:
        case PkgType.PKG_DOCS:
            break;
        default:
            return null;
        }
    ...
    public LocalPkgInfo[] getPkgsInfos(int filters) {
        ...
        for (int filter = minf; filters != 0 && filter <= PkgType.PKG_ALL; filter <<= 1) {
            ...
            case PkgType.PKG_BUILD_TOOLS:
            case PkgType.PKG_PLATFORMS:
            case PkgType.PKG_SYS_IMAGES:
            case PkgType.PKG_ADDONS:
            case PkgType.PKG_SAMPLES:
            case PkgType.PKG_SOURCES:
            case PkgType.PKG_EXTRAS:
                Collection<LocalPkgInfo> existing = mLocalPackages.get(filter);
                if (existing.size() > 0) {
