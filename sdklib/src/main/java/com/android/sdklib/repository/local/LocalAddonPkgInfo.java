...
public class LocalAddonPkgInfo extends LocalPlatformPkgInfo {
    ...
    public LocalAddonPkgInfo(@NonNull LocalSdk localSdk,
        ...
        super(localSdk, localDir, sourceProps, version, revision, PreciseRevision.NOT_SPECIFIED);
        if (sourceProps.contains(PkgProps.PKG_ID)) {
            mAddonDesc = PkgDesc.Builder
                    .newGeneric(PkgType.PKG_ADDON, sourceProps.getProperty(PkgProps.ADDON_NAME_ID),
     ...
     * system image. If any given API appears twice or more, the first occurrence wins. <p/> Note
     * that it's OK for an add-on to have no system-images at all, since it can always rely on the
     * ones from its base platform.
     *
