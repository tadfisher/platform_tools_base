...
public class GradleImportTest extends TestCase {
    ...
    private static File createLibrary(File root, String lib2Pkg) throws IOException {
        ...
        createProjectProperties(lib1, "android-19", null, true, null,
                Collections.<String>singletonList(".." + separator + javaLibRelative));
        ...
        createProjectProperties(lib2, "android-18", null, true, null,
                Collections.<String>singletonList(".." + separator + lib1Name));
    ...
    private static void createProjectProperties(
            ...
            @NonNull List<String> libraries) throws IOException {
        StringBuilder sb = new StringBuilder();
        ...
        if (mergeManifest != null) {
            sb.append("manifestmerger.enabled=").append(mergeManifest.booleanValue()).append("\n");
        }

        if (isLibrary != null) {
            sb.append("android.library=").append(isLibrary.booleanValue()).append("\n");
        }

        // The libraries parameter is deliberately a string rather than a File, because
        // when I earlier had a List<File> and constructed relative paths using
        // new File(".." + Files.separator + "something"), I got weird results on Windows;
        // I suspect the File constructor's call to fs.normalize on Windows does some resolution
        // for relative paths which doesn't work right in this context.
        for (int i = 0, n = libraries.size(); i < n; i++) {
            String path = libraries.get(i);
            sb.append("android.library.reference.").append(Integer.toString(i)).append("=").
                    append(path.replace('/', separatorChar)).append("\n");
        }
