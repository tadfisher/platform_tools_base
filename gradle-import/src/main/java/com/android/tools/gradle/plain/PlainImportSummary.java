 ...
 *     </ul>
 */
public class PlainImportSummary {
    ...
    private Map<PlainImportModuleBlah,Map<File,File>> mMoved = Maps.newHashMap();
    ...
    PlainImportSummary(@NonNull PlainGradleImport importer) {
        mImporter = importer;
    }
    ...
    public void reportMoved(@NonNull PlainImportModuleBlah module, @NonNull File from,
            @NonNull File to) {
    ...
    public String createSummary() {
        ...
        if (!mMoved.isEmpty()) {
            ...
            List<PlainImportModuleBlah> modules = Lists.newArrayList(mMoved.keySet());
            Collections.sort(modules);
            for (PlainImportModuleBlah module : modules) {
                if (modules.size() > 1) {
                ...
                for (File from : sorted) {
                    ...
                    try {
                        ...
                        if (mDestDir != null) {
                            toRelative = PlainGradleImport.computeRelativePath(mDestDir.getCanonicalFile(), to);
                        }
