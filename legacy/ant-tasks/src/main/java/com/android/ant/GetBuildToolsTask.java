...
public class GetBuildToolsTask extends Task {
    ...
    public void execute() throws BuildException {
        ...
        if (buildToolInfo == null) {
            ...
            Set<FullRevision> fullRevisionSet = sdkManager.getBuildTools();

            if (fullRevisionSet.isEmpty()) {
                throw new BuildException("SDK does not have any Build Tools installed.");
            }

            List<FullRevision> fullRevisionList = Lists.newArrayList(fullRevisionSet);
            Collections.sort(fullRevisionList);

            FullRevision fullRevision = fullRevisionList.get(fullRevisionList.size() - 1);
            System.out.println("Using latest Build Tools: " + fullRevision);


            buildToolInfo = sdkManager.getBuildTool(fullRevision);
        }
